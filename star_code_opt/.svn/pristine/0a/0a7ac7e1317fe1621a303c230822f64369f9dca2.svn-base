 ...
 *Modification 			: Added by shiv for showing Group Travel in case of group travel inspite of traveller Name on  02-Jul-08;  
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<ul class="pagebullet">
      <li class="pageHead"> <b><%=dbLabelBean.getLabel("label.allrequisitions.domesticrequisitions",strsesLanguage) %></b></li>
    </ul></td>
...
<table width="100%" align="center" border="0" cellpadding="5" cellspacing="1"  class="formborder">  
 <form method=post name=frm action="All_Requisitions_D.jsp" onSubmit="return checkForm();">
   ...
   <tr> 
     <td class="formtr3" align=center>
	   <select name="chkData"  class="textboxCSS" onchange="blankTextBox('<%=stroldDate %>','<%=strCurrDate %>');">
...
{    
    ...
    sSqlStr=   "SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,TD.TRAVEL_REQ_NO, "+
	           "RTRIM( m_userinfo.firstname)+ ' ' + RTRIM(m_userinfo.middlename) +' ' + RTRIM(m_userinfo.lastname) AS "+ 
               " TRAVELLER, case when year(TD.C_DATETIME)=1900 then ' ' else convert(varchar(10),TD.C_DATETIME,103) end AS "+ 
		   " C_DATETIME,RTRIM( m_userinfo1.firstname)+ ' ' + RTRIM(m_userinfo1.middlename) +' ' + RTRIM(m_userinfo1.lastname) "+
           " AS CREATED_BY, isnull(TRAVEL_REQ_COMMENTS.TRAVEL_ID,0) AS TRAVEL_COMMENT, "+
	       " case when year(TD.TRAVEL_DATE)=1900 then ' ' else convert(varchar(10),TD.TRAVEL_DATE,103) end AS TRAVEL_DATE, "+
           " ISNULL(CONVERT(decimal(20,0),TD.TOTAL_AMOUNT),'0') AS TOTAL_AMOUNT,isnull(TRAVEL_FROM,'-') AS "+
		" TRAVEL_FROM,isnull(TRAVEL_TO,'-') AS TRAVEL_TO,isnull(TRAVEL_ATTACHMENTS.ATTACHMENT_ID,0) as ATTACHMENT, "+ 
		" TD.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG,(select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id FROM T_APPROVERS TA inner join  T_TRAVEL_STATUS TS "+
	    " on  TA.TRAVEL_ID=TS.TRAVEL_ID inner join T_TRAVEL_DETAIL_DOM TD on TD.TRAVEL_ID=TS.TRAVEL_ID left join m_userinfo on " + 
		" m_userinfo.USERID=TA.TRAVELLER_ID left join m_userinfo m_userinfo1 on m_userinfo1.USERID=TA.C_USER_ID "+
		" left join TRAVEL_REQ_COMMENTS on TRAVEL_REQ_COMMENTS.TRAVEL_ID=TA.TRAVEL_ID and TRAVEL_REQ_COMMENTS.TRAVEL_TYPE='D' "+
		" left join T_JOURNEY_DETAILS_DOm on T_JOURNEY_DETAILS_DOM.TRAVEL_ID= TA.TRAVEL_ID  AND  JOURNEY_ORDER=1 " +
		" left join TRAVEL_ATTACHMENTS  on TRAVEL_ATTACHMENTS.TRAVEL_ID=TA.TRAVEL_ID and TRAVEL_ATTACHMENTS.TRAVEL_TYPE='D' "+
	   "  WHERE TS.TRAVEL_TYPE='D' AND TA.TRAVEL_TYPE='D' AND TS.TRAVEL_STATUS_ID IN(2,10,11)   "+
	   " AND TA.SITE_ID="+strSiteIdSS+" AND BILLING_SITE !='0'   ORDER BY C_DATETIME DESC";  // AND TA.APPROVE_STATUS=10 AND TA.APPROVER_ID='"+Suser_id+"' AND TS.TRAVEL_STATUS_ID=2";
 ...
 else
 {
	...
	sSqlStr          =  "SET DATEFORMAT DMY SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY, DBO.TRAVEL_COMMENT(TA.TRAVEL_ID,'D')AS TRAVEL_COMMENT, ISNULL(dbo.CONVERTDATEDMY1(TD.TRAVEL_DATE),'-') AS TRAVEL_DATE, ISNULL(CONVERT(decimal(20,0),TD.TOTAL_AMOUNT),'0') AS TOTAL_AMOUNT, ISNULL(DBO.TRAVEL_FROM(TA.TRAVEL_ID,'D'),'-') AS TRAVEL_FROM, ISNULL(DBO.TRAVEL_TO(TA.TRAVEL_ID,'D'),'-') AS TRAVEL_TO, DBO.REQUISITIONATTACHMENT(TD.travel_id,'D') AS ATTACHMENT,TD.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG,(select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_DOM TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID AND TS.TRAVEL_TYPE='D' AND TA.TRAVEL_TYPE='D' AND TS.TRAVEL_STATUS_ID IN(2,10,11) AND TA.SITE_ID="+strSiteIdSS+" AND BILLING_SITE !='0'  AND CAST(FLOOR(CAST(TD.C_DATETIME AS FLOAT))AS DATETIME) BETWEEN '"+strFrom+"' AND '"+strTo+"' ORDER BY C_DATETIME DESC";  //AND  TA.APPROVE_STATUS=10 AND TA.APPROVER_ID='"+Suser_id+"' AND TS.TRAVEL_STATUS_ID=2;
				      ...
				      strTravelAgencyTypeId			=	rs.getString("travel_agency_id");
								...
								{		
									strTraveller="Group/Guest Travel ";  
  									urlPage="Group_T_TravelRequisitionDetails_D.jsp";
								  ...
								  {
										urlPage="T_TravelRequisitionDetails_D.jsp";
										if(strTravelAgencyTypeId != null && strTravelAgencyTypeId.equals("2")){
...
%>
  <tr  valign="top" class="<%=strStyleCls%>"> 
    <td class="listhead" width="6%" align="center">
	  <%=intSerialNo%><br>
	   <a href="#" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=D&whichPage=All_Requisitions_D.jsp&targetFrame=no','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strImagePrint%></a>
	  <a href="#" title="<%=dbLabelBean.getLabel("link.createrequestlist.attachments",strsesLanguage) %>" onClick="MM_openBrWindow('requisitionAttachment.jsp?purchaseReqID=<%=strTravelId%>&travel_type=I&err=0&whichPage=All_Requisitions_D.jsp&targetFrame=no','Attachments','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strAttachment%></a> 
	 </td>
    <td class="listdata" width="12%" align="center"><a href="#" 					onClick="MM_openBrWindow('<%=urlPage%>?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>&travel_type=D&travelType=D','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strTravelRequestNo%></A></td>
	...
	<td class="listhead" width="10%" align="CENTER" nowrap="nowrap">
	  ...
	  <a href="#" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=D&whichPage=All_Requisitions_D.jsp&targetFrame=no','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%></a> |<br>
	  <a href="#" onClick="MM_openBrWindow('UnderConstruction.html?travel_id=<%=strTravelId%>','HELP','scrollbars=yes,resizable=yes,width=600,height=316')"><%=dbLabelBean.getLabel("label.approverequest.history",strsesLanguage)%></a>
	</td>
  </tr>
    <%
  intSerialNo++;
