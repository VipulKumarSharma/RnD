  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td width="70%" height="38" class="bodyline-top">
	    <ul class="pagebullet"><li class="pageHead"><%=dbLabelBean.getLabel("label.user.starsactivelocaladmin",strsesLanguage)%> <font color='red'><%=strMessage%></font></li>
	    </ul>
	  </td>
      <td width="30%" align="right" valign="bottom" class="bodyline-top">
          ...
          <tr align="right">
			<td>
			<ul id="list-nav">
			<li><a href="M_LocalAdminDeactiveUserList.jsp"><%=dbLabelBean.getLabel("label.master.deactivated",strsesLanguage)%></a></li>
...
<%
    ...
    sSqlStr="SELECT DISTINCT M2.USERID, DBO.DIVISIONNAME(DIV_ID) AS DIV,DBO.SITEDETAILS(M2.SITE_ID) AS SITE, ISNULL(DBO.DEPARTMENTNAME(DEPT_ID),'-') AS DEPT, RTRIM(DBO.USER_NAME(M2.USERID)) AS UNAME,DBO.CONVERTDATE(C_DATE) AS CREATEDDATE,USERNAME ,EMAIL, ISNULL(M2.ROLE_ID,'') AS ROLE,PIN,ISNULL(DBO.DESIGNATIONNAME(DESIG_ID),'') AS DESIG_ID, LTRIM(RTRIM(ISNULL(APPROVER_LEVEL,'0'))) AS APPROVER_LEVEL,M2.SITE_ID FROM M_USERINFO M1, M_USERROLE M2 WHERE M1.USERID = M2.USERID AND M2.STATUS_ID=10 AND APPLICATION_ID=1  ORDER BY 1";
...
<!--Changed on 17 May by Sachin end-->
	...
	<a href="M_LocalAdminEdit.jsp?userId=<%=strUserId%>&sitedivId=<%=sSiteId%>"><%=dbLabelBean.getLabel("link.createeditlist.edit",strsesLanguage)%></a> |<br><a href="M_LocalAdminActiveDeactive.jsp?userId=<%=strUserId%>&sitedivId=<%=sSiteId%>&statusId=30" onClick="return  deleteConfirm();"><%=dbLabelBean.getLabel("link.master.deactive",strsesLanguage)%></a>  <!-- changed by shiv  on 17 April  07  passing  sitedivId  for deactivate Local Admin  -->
	</td>
  </tr>
<%
