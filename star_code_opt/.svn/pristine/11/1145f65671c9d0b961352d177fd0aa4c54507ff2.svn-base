 ...
 try
 {
	...
	// {
		...
		try
    	{
	    	...
	    	sqlstr="SELECT MAIL_ID,ISNULL(DBO.FN_REMOVE_DUMMYUSER_EMAIL(RTRIM(RECEIPENT_TO),';'),'NA') AS RECEIPENT_TO,ISNULL(RTRIM(RECEIPENT_FROM),'NA') AS RECEIPENT_FROM,ISNULL(DBO.FN_REMOVE_DUMMYUSER_EMAIL(RTRIM(RECEIPMENT_CC),';'),'NA') AS RECEIPMENT_CC,RTRIM(MAIL_SUBJECT),RTRIM(MAIL_MSG),TRIES,ERROR_SUCCESS,RTRIM(MAIL_MSG) FROM  REQ_MAILBOX (NOLOCK) WHERE ERROR_SUCCESS IN ('NEW','ERROR') AND TRIES<=50 ";
			...
			while(rs.next())
			{
				...
				if(iTrials>=50)
				{
					...
					strMailFrom			="gurmeet.virdi@mind-infotech.com";
					strMailTo			="gurmeet.virdi@mind-infotech.com";
				}
				try
				{
					//Send Mails for TO
					...
					if (!strMailTo.equals("NA")) {
						StringTokenizer st1=new StringTokenizer(strMailTo,";");
						while(st1.hasMoreElements()) {							
						   msg.addRecipient(Message.RecipientType.TO, new InternetAddress(st1.nextToken()));
						}
					}
					...
					mbp.setContent(strMailBody,"text/html;charset=UTF-8");
					...
					msg.setHeader("X-Priority", "1"); 
	     			...
	     			if(!((strMailTo.equals("") || strMailTo.equals("NA")) && (strMailCc.equals("") || strMailCc.equals("NA")))){
	     				Transport.send(msg);
					}
