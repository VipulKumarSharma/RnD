 ...
 *Modification 			: 1 Add Cancel link .
                          2 Added new flage  for vaman sehgal to view him requisition on 04-11-2008 by shiv sharma 
                          3 added code for showing travel class of journey on 29 march 2010 by shiv sharma 
 ...
 *Modification			:Disable checkboxes for those international type of request for which local price is entered.
 ...
 *Modification			:Showing international requests for reviewers to either view or take action
...
function childwindowSubmit(){
	document.frm.action='T_approveTravelRequisitions.jsp';
	document.frm.submit();
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<%if(ssflage.equals("3")){ %>
     ...
     <td align="right" class="bodyline-top" valign="bottom"><table align="right" border="0" cellspacing="0" cellpadding="0">
       <tr align="right" valign="bottom">
       <td>
       <ul id="list-nav">
<%if(SuserRole!=null &&  !SuserRole.equals("MATA") ){%>
<li><a href="#" onClick="MM_openBrWindow('T_MyReturns.jsp?flag=1&travel_type=I','REQUISITIONSRETURNED','scrollbars=yes,resizable=yes,width=900,height=500');"><%=dbLabelBean.getLabel("button.approverequest.requisitionsreturned",strsesLanguage) %></a></li>
<%}%>
<li><a href="#" onClick="MM_openBrWindow('MyApproved.jsp?flag=1&travel_type=I','REQUISITIONSAPPROVED','scrollbars=yes,resizable=yes,width=900,height=500');"><%=dbLabelBean.getLabel("button.approverequest.requisitionsapproved",strsesLanguage) %></a></li>
<li><a href="T_transferMain.jsp?travel_type=I" ><%=dbLabelBean.getLabel("button.approverequest.handoverrequisitions",strsesLanguage) %></a></li>
<li><a href="#" onClick="MM_openBrWindow('T_searchTravelRequisitions.jsp?travel_type=I','SEARCH','scrollbars=yes,resizable=yes,top=0,left=0,width=1000,height=565');"><%=dbLabelBean.getLabel("button.global.search",strsesLanguage) %></a></li>
...
<%if(SuserRole!=null && SuserRole.equals("MATA") && strOrderID.equals("10.0")){%>
  <tr>
    <td colspan="15" class="formhead" >New Request (s)</td>
  </tr>
<%}%>
...
//if(strApproverRole!=null && ( strApproverRole.equals("CHAIRMAN") || strApproverRole.equals("MATA")   ) )
/*if(SuserRole!=null && ( SuserRole.equals("CHAIRMAN") || SuserRole.equals("MATA") ||  SglobalApproverRole.equals("global")  ) )
{*/
...
strSql="SELECT DISTINCT  TA.TRAVEL_ID, TA.TRAVELLER_ID, TD.TRAVEL_REQ_NO,"+
		...
		" td.GROUP_TRAVEL_FLAG AS GROUP_TRAVEL_FLAG,'Y' AS FLAG ,TA.PAP_FLAG "+
		" ,(select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id, "+
		...
		" ISNULL(RTRIM(.DBO.NEXTAPPROVER(TD.TRAVEL_ID, 'I', TA.TRAVELLER_ID)), 'NA') AS NEXTWITH,"+
		" DBO.REQUISITIONATTACHMENT(TD.TRAVEL_ID, 'I') AS ATTACHMENT, DBO.TRAVEL_COMMENT(TD.TRAVEL_ID, 'I') AS COMMENT,"+
		" td.GROUP_TRAVEL_FLAG AS GROUP_TRAVEL_FLAG, case when access_level='full' then 'S' else 'N' end AS FLAG ,TA.PAP_FLAG "+
		" , (select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id, "+
		" RTRIM(LTRIM(DBO.SITENAME(TD.SITE_ID)))+CASE WHEN TD.SITE_ID IN (86,297) AND DBO.USEREMAIL(TA.TRAVELLER_ID) "+
		" LIKE '%@MINDID.COM%' THEN '-Design' WHEN TD.SITE_ID IN (86,297) AND DBO.USEREMAIL(TA.TRAVELLER_ID) "+
		" LIKE '%@MIND%' THEN '-SW' else '' end AS TRAVELSITE FROM  T_APPROVERS TA inner join"+
...
//query modified by manoj chand on 14 feb 2012 to get tk_provider_flag
   // strSql =  "SELECT ISNULL(DBO.CONVERTDATEDMY1(TD.TRAVEL_DATE),'-'),dbo.FN_GET_EXPENDITURE('"+strTravelId+"','I') AS Expenditure, ISNULL(DBO.TRAVEL_FROM("+strTravelId+",'I'),'-') AS TRAVEL_FROM, ISNULL(dbo.TRAVEL_TO("+strTravelId+",'I'),'-') AS TRAVEL_TO,dbo.FN_TRAVEL_CLASS("+strTravelId+",'I') as travel_class,c_userid,tk_provider_flag FROM T_TRAVEL_DETAIL_INT TD WHERE TD.TRAVEL_ID="+strTravelId ;   
    ...
    //Added new flage for vaman sehgal to view him requisition on 04-11-2008
 ...
 <tr VALIGN="TOP" class="<%=strStyleCls%> int-req-rows"> 
    <td width="3%" align="Left" bgColor="<%=strColor%>">
     <% //Added new Code  for vaman sehgal to view him requisition on 04-11-2008  by shiv sharma 
      ...
      %>
    <input class="<%=strStyleCls%>" type=checkbox name="chk<%=intSerialNo++%>" value="<%=strTravelId%>;<%=strTravellerId%>;I" <%=strCheckBoxDisabled%> >
    </td>
     
    <td class="listhead" width="3%" align="Left"> <%=intSerialNo%>  <% if (strViewflage.equals("Y")){ %><a href="#" title="<%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage) %>" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=I&whichPage=T_approveTravelRequisitions.jsp&targetFrame=yes','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strComment%></a> <a href="#" title="<%=dbLabelBean.getLabel("label.global.addattachments",strsesLanguage)%>" onClick="MM_openBrWindow('requisitionAttachment.jsp?purchaseReqID=<%=strTravelId%>&err=0&travel_type=I&whichPage=T_approveTravelRequisitions.jsp&targetFrame=yes','Attachments','scrollbars=yes,resizable=yes,width=875,height=550')"><%=strAttachment%></a>
        <%} %>
     </td>	
    ...
    if(SuserRole!=null && SuserRole.equals("MATA") )
    {
		...
		%>
	 	 <td class="listdata" width="5%" align="center">
			<a href="#" onClick="MM_openBrWindow('T_reqFrame_Receive_MATA.jsp?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>&travel_type=I','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strTravelRequestNo%></a> 
	 	 </td>
		<%  }
	else 
	{
...
//else if message added by manoj chand to implement special workflow
	 ...
	 <td class="listdata" width="5%" align="center">
		<a href="#" onClick="MM_openBrWindow('T_reqFrame.jsp?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>&travel_type=I&spFlag=y','SEARCH','scrollbars=yes,resizable=yes,width=775,height=850')"><%=strTravelRequestNo%></a> 
		</td>
 <%}else{	
	 //Added new Code  for vaman sehgal to view him requisition on 04-11-2008  by shiv sharma
	 if(strGroupTravelFlag!=null  &&  (strGroupTravelFlag.trim().equals("Y")))   
	  ...
	  %>
	 <td class="listdata" width="5%" align="center">
		<a href="#" onClick="MM_openBrWindow('<%=strPageUrl%>?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>&travel_type=I','SEARCH','scrollbars=yes,resizable=yes,width=575,height=450')"><%=strTravelRequestNo%></a> 
	  </td>
	  <%
 } 
%>
...
<%--     <td class="listdata" width="11%" align="center"><%=strTo%></td> --%>
    ...
    <td class="listhead" width="19%" align="center" nowrap="nowrap">
 	  <!--<a  href="T_IntTrv_Details_Edit.jsp?travelId=<%=strTravelId%>">Edit</a>  |-->   
 	  <% //Added new Code  for vaman sehgal to view him requisition on 04-11-2008  by shiv sharma
 	    if(strViewflage.equals("Y")){%>
      <a href="#" onClick="MM_openBrWindow('requisitionAttachment.jsp?purchaseReqID=<%=strTravelId%>&err=0&travel_type=I&whichPage=T_approveTravelRequisitions.jsp&targetFrame=yes','Attachments','scrollbars=yes,resizable=yes,width=875,height=550')" title="<%=dbLabelBean.getLabel("label.global.addattachments",strsesLanguage)%>"><!--<img src="images/clip123.gif?buildstamp=2_0_0" border=0>--><%=dbLabelBean.getLabel("link.approverequest.attach",strsesLanguage)%></a> |<br> 
...
<%
	 ...
	 {
%>
       | <a href="#" onClick="MM_openBrWindow('T_Travel_Requisition_Cancel.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=I&whichPage=T_approveTravelRequisitions.jsp&targetFrame=yes','CANCEL','scrollbars=yes,resizable=yes,width=900,height=260')"><%=dbLabelBean.getLabel("link.approverequest.cancel",strsesLanguage)%></a>	   
<%
	 }
...
// THIS FOLLOEING  CODE IS COMMENTED SO MATA PERSON CAN NOT SEE REQUEST  IN RECIVED OPTION ON 03-APR-08  BY SHIV SHARMA  
// THIS FOLLOEING CODE IS COMMENTED SO MATA PERSON CAN NOT SEE REQUEST  IN RECIVED OPTION ON 03-APR-08  BY SHIV SHARMA  
// THIS FOLLOEING CODE IS COMMENTED SO MATA PERSON CAN NOT SEE REQUEST  IN RECIVED OPTION ON 03-APR-08  BY SHIV SHARMA  
// THIS FOLLOEING CODE IS COMMENTED SO MATA PERSON CAN NOT SEE REQUEST  IN RECIVED OPTION ON 03-APR-08  BY SHIV SHARMA  
// THIS FOLLOEING CODE IS COMMENTED SO MATA PERSON CAN NOT SEE REQUEST  IN RECIVED OPTION ON 03-APR-08  BY SHIV SHARMA  
// THIS FOLLOEINGCODE IS COMMENTED SO MATA PERSON CAN NOT SEE REQUEST  IN RECIVED OPTION ON 03-APR-08  BY SHIV SHARMA  
...
<%
  ...
  strComment   ="0";
  int intCount1 = 1; 
  //strSql="SELECT DISTINCT TD.TRAVEL_ID, TD.TRAVELLER_ID, TD.TRAVEL_REQ_NO,ISNULL(RTRIM(.DBO.USER_NAME(TD.TRAVELLER_ID)),'NA') AS TRAVELLER,ISNULL(DBO.CONVERTDATEDMY1(TD.C_DATETIME),'-') AS C_DATE, ISNULL(RTRIM(.DBO.USER_NAME(TD.C_USERID)),'NA') AS CREATED_BY, ISNULL(RTRIM(.DBO.NEXTAPPROVER(TD.TRAVEL_ID,'I',TD.TRAVELLER_ID)),'NA') AS NEXTWITH, DBO.REQUISITIONATTACHMENT(TD.TRAVEL_ID,'I')AS ATTACHMENT,DBO.TRAVEL_COMMENT(TD.TRAVEL_ID,'I') AS COMMENT  FROM T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TD.TRAVEL_ID=TS.TRAVEL_ID AND TD.STATUS_ID=10  AND TS.TRAVEL_STATUS_ID=10 AND TS.TRAVEL_TYPE='I'";


///THIS QUERY WILL NOT GIVE ANY RESULT BECOZ  TS.TRAVEL_STATUS_ID=80  DONE BY SHIV SAHRMA ON 03-Apr-08

strSql ="SELECT DISTINCT TD.TRAVEL_ID, TD.TRAVELLER_ID, TD.TRAVEL_REQ_NO,ISNULL(RTRIM(.DBO.USER_NAME(TD.TRAVELLER_ID)),'NA') AS TRAVELLER,ISNULL(dbo.CONVERTDATEDMY1(TD.C_DATETIME),'-') AS C_DATE, ISNULL(RTRIM(.DBO.USER_NAME(TD.C_USERID)),'NA') AS CREATED_BY, ISNULL(RTRIM(.DBO.NEXTAPPROVER(TD.TRAVEL_ID,'D',TD.TRAVELLER_ID)),'NA') AS NEXTWITH, dbo.REQUISITIONATTACHMENT(TD.TRAVEL_ID,'D')AS ATTACHMENT,dbo.TRAVEL_COMMENT(TD.TRAVEL_ID,'I') AS COMMENT,ta.approver_id ,td.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG,(select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id FROM T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD,T_APPROVERS TA WHERE TD.TRAVEL_ID=TS.TRAVEL_ID  AND TD.TRAVEL_ID=TA.TRAVEL_ID AND TD.STATUS_ID=10  AND TS.TRAVEL_STATUS_ID=80 AND TS.TRAVEL_TYPE='I'  and ta.travel_type='I'  and ta.approve_status=10 and ta.approver_id="+Suser_id;
  ...
  while(rs.next())
  {
    ...
    //strSql =  "SELECT ISNULL(DBO.CONVERTDATEDMY1(TD.TRAVEL_DATE),'-'),dbo.FN_GET_EXPENDITURE('"+strTravelId+"','I') AS Expenditure,ISNULL(dbo.TRAVEL_FROM("+strTravelId+",'I'),'-') AS TRAVEL_FROM, ISNULL(dbo.TRAVEL_TO("+strTravelId+",'I'),'-') AS TRAVEL_TO FROM T_TRAVEL_DETAIL_INT TD WHERE TD.TRAVEL_ID="+strTravelId ;   
	
    strSql =  "SELECT ISNULL(DBO.CONVERTDATEDMY1(TD.TRAVEL_DATE),'-'),dbo.FN_GET_EXPENDITURE('"+strTravelId+"','I') AS Expenditure, [dbo].[FN_GetJourneyDetailsINT]("+strTravelId+") AS TRAVEL_FROM, ISNULL(dbo.TRAVEL_TO("+strTravelId+",'I'),'-') AS TRAVEL_TO FROM T_TRAVEL_DETAIL_INT TD WHERE TD.TRAVEL_ID="+strTravelId ;   
...
%>
 ...
 <tr VALIGN="TOP" class="<%=strStyleCls%>"> 
    ...
    <td class="listhead" width="3%" align="Left"><%=intCount1++%><a href="#" title="<%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage) %>" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=I&whichPage=T_approveTravelRequisitions.jsp&targetFrame=yes','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strComment%></a> <a href="#" title="<%=dbLabelBean.getLabel("label.global.addattachments",strsesLanguage)%>" onClick="MM_openBrWindow('requisitionAttachment.jsp?purchaseReqID=<%=strTravelId%>&err=0&travel_type=I&whichPage=T_approveTravelRequisitions.jsp','Attachments','scrollbars=yes,resizable=yes,width=875,height=550')"><%=strAttachment%></a> </td>
	

    <td class="listdata" width="5%" align="center"><a href="#" onClick="MM_openBrWindow('T_reqFrame_MATA.jsp?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>&travel_type=I','SEARCH','scrollbars=yes,resizable=yes,width=775,height=850')"><%=strTravelRequestNo%></a> </td>
...
%>
  <tr>
    <td colspan="15" class="formtr2"><!-- No Records Found --></td>
  </tr>
<%
  }
}
...
<%	}%>
	    <input type="button" name="CheckAll" id="CheckAll" value="<%=dbLabelBean.getLabel("button.approverequest.checkall",strsesLanguage)%>" onClick="checkAll(document.frm)" class="formButton" >
