...
<html lang="en">
	<head>
		...
		<script type="text/javascript" language="JavaScript" src="jquery-datepicker/jquery.datepick.js?buildstamp=2_0_0" ></script>
	 	...
	 	<script type="text/javascript" src="EditableSelectBox/js/jquery.editable-select.js" ></script>
...
<body>
	...
	<%	} %>		
			<table width="99.3%" align="left" border="0" cellpadding="5" cellspacing="1" class="formborder" id="personalInfoTable">
				...
				<%	} %>	
					...
					<td><%=dbLabelBean.getLabel("label.personnelbooking.forex",strsesLanguage)%></td>
				</tr>
				</thead>
				<tbody>
				<%
				...
				%>
					...
					<td style="width:180px;">
						<input type="text" name="paxName" id="paxName" value="<%=paxName%>" class="tableTextBox" onkeyup="return test1(this,200,'c.');" onfocus="initializePaxName(this);" style="width:100%;text-align: center;">
					</td>
					<td style="width:90px;">
				...
				%>
					...
					<td style="width:180px;">
						<input type="text" name="paxName" id="paxName" value="" class="tableTextBox" onkeyup="return test1(this,200,'c.');" onfocus="initializePaxName(this);" style="width:100%;">
					</td>
					<td style="width:90px;">
	...
	<%	} else { } %>
			...
			<div id="tktInfo" class="hide">
				...
				<input type="hidden" name="hidTicketDataFlag" value="<%=tktReqDetailExists%>"/>
	...
	<script>
						...
						{	for (var i = 0, len = rowCountTkt; i < len; i++) 
							{	if(secFrom[i].value != '' && secFrom[i].value != '<%=dbLabelBean.getLabel("label.global.departurecity",strsesLanguage)%>') 
								{	
									...
									if(pnr[i].value=='' && ticketNo[i].value=='') 
									{	alert('PNR No. / Ticket No.');
						...
						{	
							if (document.getElementById('chkbox'+i).checked) 
							{
								...
								{	if(cnclSecFrom[i].value=='' || cnclSecFrom[i].value=='<%=dbLabelBean.getLabel("label.global.departurecity",strsesLanguage)%>') 
									...
									{	alert('Ticket Cancel Date');
										tktCnclDate[i].style.backgroundColor="#fbfebc";
										tktCnclDate[i].style.color="#000000";
										tktCnclDate[i].focus();
										return false;
									}
									if(pnrNo[i].value=='' && tktNo[i].value=='') 
									{	alert('Cancellation: PNR No. / Ticket No.');
		...
		{ 	
			...
			if(crossFlag==0)
			{
				...
				document.frm.action="Single_PersonnelBookingServlet";
		...
		function openDomInEditMode() {
			...
			{	if(subRole.indexOf("TC_TICK")>=0 || subRole.indexOf("TC_ADMI")>=0)
				...
				if(subRole.indexOf("TC_TICK")>=0 || subRole.indexOf("TC_ADMI")>=0)
				{	if (cnclRefundDetailExists==false)
					{	
						$("#cancellation").hide();
						cnclChkYFlag = false;
					}
		...
		function initializePaxName(elId) {
			var timerGuestList;
		   	...
		   	$(elId).keyup(function() { 
		   	    ...
		   	    if ($(elId).val()) {
		   	        ...
		   	        typingTimerGuestList = setTimeout(function() {
		   	        	timerGuestList = getPaxList(elId);	
		   	        }, getGuestListInterval);
		   	    } else if(!$(elId).val()) {
		   	    	$(elId).val("");
		   	    	$(elId).attr("placeholder","");
		   	    }
		   	});	   	
		}
		
		function getPaxList(elId) {
		   	...
		   	$(elId).autocomplete({
				...
				source : function(request, response) {
		            ...
		            $.ajax({
						 ...
						 data: { fName: $(elId).val(), siteId: siteId, flag:"getPaxList" },
		                 dataType: "json",
		...
		function getPaxDetail(userId) {
			var flag="getPaxDetail";
			var urlParams="flag="+flag+"&userId="+userId;
		   	...
		   	$.ajax({
		           ...
		           success: function(result) {
					...
					if(result!=null) { 
	   					...
	   					$("#paxName").val(result.PAXNAME);
	   					$("#dob").val(result.DOB);
	   					$("#mobileNo").val(result.CONTACTNO);
	   					$("#ppNo").val(result.PASSPORTNO);
	   					$("#ppExpDt").val(result.PPEXPIRYDATE);
	   					
	   					$("#paxName").css('color', '#000000');
	   					$("#dob").css('color', '#000000');
	   					$("#mobileNo").css('color', '#000000');
	   					$("#ppNo").css('color', '#000000');
	   					$("#ppExpDt").css('color', '#000000');
					 }
		          }
		     });
		}
		
		$('#paxName').on("change keyup", function() {
            if($(this).val() == "") {
            	clearPaxDetails();
