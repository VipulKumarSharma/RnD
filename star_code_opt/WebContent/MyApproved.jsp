 ...
 *Modification 			: 1.Change the view of reports in case of MATA and Others
                          2.Added by shiv for showing Group Travel in case of group travel inspite of traveller Name on  21-Feb-08 ;   
     					  3.Changes the  Query for faster response bby shiv sharma on 01/10/2008
 *Reason of Modification: 
 *Date of Modification  : 05/04/2007
 *Revision History		:
 *Modified By			: Vijay Singh
 *Editor				:Editplus
 *Modified By			:Manoj Chand
 *Date of Modification	:20 Dec 2011
 *Modification			:to resolve javascripting error coming on close button of comments screen.
 *******************************************************/
%>
...
//function added by manoj chand on 20 dec 2011 to resolve javascripting error
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<ul class="pagebullet">
	<% if(ssflage.equals("3")){%>
      <li class="pageHead"> <b><%=dbLabelBean.getLabel("label.approverequest.intercontrequisitionsapprovedbyme",strsesLanguage)%></b></li>
      <%}else{ %>
      <li class="pageHead"> <b><%=dbLabelBean.getLabel("label.approverequest.internationalrequisitionsapprovedbyme",strsesLanguage)%></b></li>
      <%} %>
    </ul></td>
...
<table width="100%" align="center" border="0" cellpadding="5" cellspacing="0"  class="formborder">  
 <form method=post name=frm action="MyApproved.jsp" onSubmit="return checkForm();">
   ...
   <tr> 
     ...
     <td class="formhead" width="30%" align="center"><img src="images/help.gif?buildstamp=2_0_0" width="44" height="16" border="0" onClick="MM_openBrWin('help_approvedBy Me.html#300a','','scrollbars=yes,resizable=yes,width=700,height=300')"></td>
   </tr>
   <tr> 
...
rs.close();
  ...
  {    
	...
	/* if(SuserRole!=null && (SuserRole.equals("CHAIRMAN") || SuserRole.equals("MATA") || SglobalApproverRole.equals("global")  ) )
     {*/
       sSqlStr=   "SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,DBO.TRAVEL_COMMENT(TA.TRAVEL_ID,'I')AS TRAVEL_COMMENT,TD.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG,(select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_TYPE='I' AND TA.TRAVEL_TYPE='I' AND TA.APPROVE_STATUS=10 AND TA.APPROVER_ID='"+Suser_id+"' AND TS.TRAVEL_STATUS_ID<>1";  // AND TS.TRAVEL_STATUS_ID=2";
	 /*}
	 else
	 {
		 sSqlStr=   "SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,dbo.TRAVEL_COMMENT(TA.TRAVEL_ID,'I')AS TRAVEL_COMMENT FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_TYPE='I'  AND TA.TRAVEL_TYPE='I'      AND TA.APPROVE_STATUS=10 AND TA.APPROVER_ID='"+Suser_id+"'  AND TA.SITE_ID="+strSiteIdSS+" AND TS.TRAVEL_STATUS_ID<>1";  //+" AND TS.TRAVEL_STATUS_ID=2 ;
	 }*/
 ...
 else
 {
	...
	/*if(SuserRole!=null && (SuserRole.equals("CHAIRMAN") || SuserRole.equals("MATA") || SglobalApproverRole.equals("global") ) )
    {*/
	  sSqlStr          =  "SET DATEFORMAT DMY SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY, DBO.TRAVEL_COMMENT(TA.TRAVEL_ID,'I')AS TRAVEL_COMMENT,TD.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG,(select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id  FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID AND TS.TRAVEL_TYPE='I' AND TA.TRAVEL_TYPE='I' AND TS.TRAVEL_STATUS_ID<>1 AND  TA.APPROVE_STATUS=10 AND TA.APPROVER_ID='"+Suser_id+"' AND CAST(FLOOR(CAST(APPROVE_DATE AS FLOAT))AS DATETIME) BETWEEN '"+strFrom+"' AND '"+strTo+"'";  //AND TS.TRAVEL_STATUS_ID=2;
	/*}
	else
	{
       sSqlStr          =  "SET DATEFORMAT DMY SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,DBO.TRAVEL_COMMENT(TA.TRAVEL_ID,'I')AS TRAVEL_COMMENT FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_TYPE='I'       AND TA.TRAVEL_TYPE='I'       AND TA.APPROVE_STATUS=10 AND TS.TRAVEL_STATUS_ID<>1 AND TA.APPROVER_ID='"+Suser_id+"' AND TA.SITE_ID="+strSiteIdSS+" AND CAST(FLOOR(CAST(APPROVE_DATE AS FLOAT))AS DATETIME) BETWEEN '"+strFrom+"' AND '"+strTo+"'"; //AND TS.TRAVEL_STATUS_ID=2
	}*/	
 }
...
//System.out.println("sSqlStr====1="+sSqlStr);
 ...
 while(rs.next())
 {
	...
	//System.out.println("sSqlStr====2.1="+sSqlStr);
	...
	sSqlStr   ="SELECT  CASE WHEN COUNTALL=COUNTAPP THEN 'APPROVED BY ALL' WHEN " + 
				...
				" FROM (SELECT DISTINCT     T_TRAVEL_DETAIL_INT.TRAVEL_ID, T_TRAVEL_STATUS.TRAVEL_TYPE, T_TRAVEL_DETAIL_INT.TRAVELLER_ID,"+
				" (SELECT COUNT(1) FROM T_APPROVERS TA(NOLOCK) WHERE TA.TRAVEL_ID=T_TRAVEL_DETAIL_INT.TRAVEL_ID  AND "+ 
				" TA.TRAVEL_TYPE=T_TRAVEL_STATUS.TRAVEL_TYPE AND TA.TRAVELLER_ID=T_TRAVEL_DETAIL_INT.TRAVELLER_ID) AS COUNTALL,"+
				" (SELECT COUNT(1) FROM T_APPROVERS TA(NOLOCK) WHERE TA.TRAVEL_ID=T_TRAVEL_DETAIL_INT.TRAVEL_ID  AND "+
				" TA.TRAVEL_TYPE=T_TRAVEL_STATUS.TRAVEL_TYPE AND TA.TRAVELLER_ID=T_TRAVEL_DETAIL_INT.TRAVELLER_ID" +
				...
				" TA.TRAVEL_ID=T_TRAVEL_DETAIL_INT.TRAVEL_ID	AND TA.TRAVELLER_ID=T_TRAVEL_DETAIL_INT.TRAVELLER_ID "+
				...
				" AND TRAVEL_STATUS_ID = 2 ORDER BY ORDER_ID ) AS PRESENTAPPROVER FROM   T_TRAVEL_DETAIL_INT(NOLOCK)"+
				" INNER JOIN T_TRAVEL_STATUS(NOLOCK) ON T_TRAVEL_DETAIL_INT.TRAVEL_ID = T_TRAVEL_STATUS.TRAVEL_ID INNER "+
				" JOIN T_APPROVERS(NOLOCK) ON T_TRAVEL_DETAIL_INT.TRAVEL_ID = T_APPROVERS.TRAVEL_ID "+ 
				" WHERE     (T_TRAVEL_DETAIL_INT.TRAVEL_ID = '"+strTravelId+"') AND (T_TRAVEL_STATUS.TRAVEL_TYPE = 'I')"+ 
				" AND (T_TRAVEL_DETAIL_INT.TRAVELLER_ID = '"+strTravellerId+"') AND (T_APPROVERS.TRAVEL_TYPE = 'I') )APPROVER_STATUS ";

	
	
	//System.out.println("sSqlStr=1111===2.2="+sSqlStr);
...
%>
  <tr  valign="top" class="<%=strStyleCls%>"> 
    <td class="listhead" width="8%" align="Left"><a href="#" title="<%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%>" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=I&whichPage=MyApproved.jsp&targetFrame=no','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strImagePrint%></a><%=intSerialNo%></td>
    

 
 <%// Vijay 05/04/2007 Change the logic in case of MATA
    if(SuserRole!=null && SuserRole.equals("MATA") )
    {
%>
	<!--@Vijay Singh  26/Mar/2007 Change the file from  T_TravelRequisitionDetails.jsp to MATA_Report_INT.jsp-->
    <td class="listdata" width="13%" align="center"><a href="#" 					onClick="MM_openBrWindow('MATA_Report_INT.jsp?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strTravelRequestNo%></A></td>
	<%}
	else 
	 {%>
	 <td class="listdata" width="13%" align="center"><a href="#" 					onClick="MM_openBrWindow('<%=urlPage%>?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>&travelType=<%=strTravelType %>','SEARCH','scrollbars=yes,resizable=yes,width=975,height=550')"><%=strTravelRequestNo%></A></td>
	<%
		 }
	 %>   
	...
	<!--<td class="listdata" width="6%" align="center"><%//=strDivName%>&nbsp;<b>/</b>&nbsp;<%//=strSiteName%></td>-->
	 <td class="listhead" width="20%" align="CENTER" nowrap="nowrap">	  
        <a href="#" title="<%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%>" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=I&whichPage=MyApproved.jsp&targetFrame=no','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%></a> <br>  <a href="#" title="<%=dbLabelBean.getLabel("label.approverequest.history",strsesLanguage)%>" onClick="MM_openBrWindow('UnderConstruction.html?travel_id=<%=strTravelId%>','HELP','scrollbars=yes,resizable=yes,width=600,height=316')"><%=dbLabelBean.getLabel("label.approverequest.history",strsesLanguage)%></a>  </td>
  </tr>
    <%
  intSerialNo++;
}
