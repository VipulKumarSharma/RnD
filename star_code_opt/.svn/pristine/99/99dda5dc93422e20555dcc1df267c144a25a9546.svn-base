...
for (int i =1 ; i < icount ; i++) {
	...
	if(strChkBoxVal.equalsIgnoreCase("Y")){	
		...
		try	{
			...
			System.out.println("Error in M_MyTeamPost.jsp ==="+e);
	    } catch(Exception e) {
	    	System.out.println("Error in M_MyTeamPost.jsp ==="+e);
	    }
...
response.sendRedirect("M_MyTeam.jsp?selectUnit="+site_ID+"&username="+user_ID+"&userFlag="+user_Flag+"&searchFlag="+strSearchFlag+"&strMessage="+URLEncoder.encode(strMessage,"UTF-8")+"");
%>