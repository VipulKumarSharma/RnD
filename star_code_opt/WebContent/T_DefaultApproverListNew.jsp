...
if(strTravelType.equals("INT"))
{
	...
	strSql = "SELECT LTRIM(RTRIM(dbo.USER_NAME(MDA.APPROVER_ID))) AS APPROVER_NAME, LTRIM(RTRIM(DBO.USER_ROLE(MDA.APPROVER_ID))) AS USERROLE, MDA.SITE_ID, MDA.ORDER_ID, ISNULL(dbo.DESIGNATIONNAME(MUI.DESIG_ID),'') AS DESIG FROM M_DEFAULT_APPROVERS MDA INNER JOIN M_USERINFO MUI ON MDA.APPROVER_ID=MUI.USERID WHERE MDA.SITE_ID="+strSiteId+" AND MDA.TRV_TYPE='I' AND  MDA.sp_role="+strWorkFlowName+" AND MDA.STATUS_ID=10 AND MDA.APPLICATION_ID=1 AND MUI.STATUS_ID=10 AND MUI.APPLICATION_ID=1 ORDER BY 4";
...
else if(strTravelType.equals("DOM"))
{
	...
	strSql = "SELECT LTRIM(RTRIM(dbo.USER_NAME(MDA.APPROVER_ID))) AS APPROVER_NAME, LTRIM(RTRIM(DBO.USER_ROLE(MDA.APPROVER_ID))) AS USERROLE, MDA.SITE_ID, MDA.ORDER_ID, ISNULL(dbo.DESIGNATIONNAME(MUI.DESIG_ID),'') AS DESIG FROM M_DEFAULT_APPROVERS MDA INNER JOIN M_USERINFO MUI ON MDA.APPROVER_ID=MUI.USERID WHERE MDA.SITE_ID="+strSiteId+" AND MDA.TRV_TYPE='D' AND  MDA.sp_role="+strWorkFlowName+" AND MDA.STATUS_ID=10 AND MDA.APPLICATION_ID=1 AND MUI.STATUS_ID=10 AND MUI.APPLICATION_ID=1 ORDER BY 4";
...
while(rs.next())
{
	...
	ApproverList.add(rs.getString("DESIG"));
}
rs.close();
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
   <tr>
     <td align=center class="formtr3">You have not defined approvers</td>
   </tr>
</table>
<%
}
...
{
%>
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="defaultApprListTbl">  
   <tr>
      <td colspan="2">
      	<table width="100%" align="center" border="0" cellpadding="1" cellspacing="1"  class="formborder">
            <form method=post name=frm action="T_MyReturns.jsp" onSubmit="return checkForm();">
              ...
              <tr>
                <td colspan="3" align="Left" class="formheadDN"><%=dbLabelBean.getLabel("label.user.defaultapprovers",strsesLanguage) %></td>
              </tr>
              <tr class="defaultAppTr">
                <td width="2%" align=center></td>
                <td width="42%" align=left class="defaultAppTrTxt"><b><%=dbLabelBean.getLabel("label.createrequest.approvers",strsesLanguage) %></b></td>
                <td width="55%" align=left class="defaultAppTrTxt"><b><%=dbLabelBean.getLabel("label.global.designation",strsesLanguage) %></b></td>
              </tr>
              <%
			...
			%>			  
			  <tr class="<%=strStyleCls%>">
                ...
                <td width="43%" align=left class="defaultAppTrTxt"><%=itr.next()%></td>
                <td width="55%" align=left class="defaultAppTrTxt"><%=itr.next()%></td>
              </tr>
			<%
				}				
			%>
