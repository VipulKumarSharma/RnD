 ...
 *Description 			:This jsp file  for showing the deleted user information to the administrator from  M_USERINFO in the STAR 
 ...
 *Modified By	        :MANOJ CHAND
 *Date of Modification  :22 August 2013
 *Description			:LTRIM,RTRIM FUNCTION APPLY ON PIN COLUMN IN SQL QUERY.
...
function submitname()
  {
	// alert("sdsd");/*
           document.frm.action="M_deletedUserList.jsp";
           frm.submit();
}
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td height="38" class="bodyline-top">
	    <ul class="pagebullet"><li class="pageHead"><%=dbLabelBean.getLabel("label.user.starsdeletedusers",strsesLanguage)%></li><font color="red"><%=strMessage%></font>
	    </ul>
	  </td>
...
//New Check for local administrator
	if((SuserRoleOther.trim().equals("LA")))
	{
       ////--------------------this query will not show the result--------------  status_id=80
		sSqlStr="SELECT USERID, dbo.DIVISIONNAME(DIV_ID) AS DIV,dbo.SITEDETAILS(SITE_ID) AS SITE,ISNULL(dbo.DEPARTMENTNAME(DEPT_ID),'-') AS DEPT,RTRIM(dbo.USER_NAME(USERID)) AS UNAME,dbo.CONVERTDATE(C_DATE) AS CREATEDDATE,USERNAME ,EMAIL,ISNULL(ROLE_ID,'') AS ROLE,LTRIM(RTRIM(PIN)) AS PIN,USERID,dbo.DESIGNATIONNAME(DESIG_ID) AS DESIG_ID,LTRIM(RTRIM(PIN)) AS PIN1, LTRIM(RTRIM(ISNULL(APPROVER_LEVEL,'0'))) AS APPROVER_LEVEL ,LTRIM(RTRIM(ISNULL(EMP_CODE,'-'))) AS EMP_CODE FROM M_USERINFO WHERE STATUS_ID=80 AND SITE_ID="+strSiteIdSS+" AND APPLICATION_ID=1  ORDER BY 1,2,3,4 ";
		...
		if(strUsername.equals(""))  
		   {
			   ...
			   strtext="and  USERNAME like '"+strUsername+"%'";      
		   }
		
		   sSqlStr="SELECT USERID, dbo.DIVISIONNAME(DIV_ID) AS DIV,dbo.SITEDETAILS(SITE_ID) AS SITE,ISNULL(dbo.DEPARTMENTNAME(DEPT_ID),'-') AS DEPT,RTRIM(dbo.USER_NAME(USERID)) AS UNAME,dbo.CONVERTDATE(C_DATE) AS CREATEDDATE,USERNAME ,EMAIL,ISNULL(ROLE_ID,'') AS ROLE,LTRIM(RTRIM(PIN)) AS PIN,USERID,dbo.DESIGNATIONNAME(DESIG_ID) AS DESIG_ID,LTRIM(RTRIM(PIN)) AS PIN1, LTRIM(RTRIM(ISNULL(APPROVER_LEVEL,'0'))) AS APPROVER_LEVEL, LTRIM(RTRIM(ISNULL(EMP_CODE,'-'))) AS EMP_CODE FROM M_USERINFO WHERE STATUS_ID=50 AND APPLICATION_ID=1 "+strtext+"  ORDER BY USERNAME";
...
<!--	  <a href="M_UpdateProfile.jsp?userId=<%=strUserId%>&flag=0&Deactiveflag=1">Edit</a> | -->
<!--	<a href="M_userActive.jsp?userId=<%=strUserId%>" onClick="return  activeConfirm();">Active</a> |-->
<!--	<a href="M_userPermanentDelete.jsp?userId=<%=strUserId%>" onClick="return  deleteConfirm();">Delete</a>-->
<!--	</td>-->
  </tr>
<%
		iSno++;				
...
<!--	<a href="M_UpdateProfile.jsp?userId=<%=strUserId%>&flag=0&Deactiveflag=1">Edit</a>|  Added a flag Deactiveflag by shiv on 24-May-07 -->
<!--	<a href="M_userActive.jsp?userId=<%=strUserId%>" onClick="return  activeConfirm();">Active</a> |-->
<!--	<a href="M_userPermanentDelete.jsp?userId=<%=strUserId%>" onClick="return  deleteConfirm();">Delete</a>-->
<!--	</td>-->
  </tr>
