 ...
 *Modification 						: 1   Added a function  for selection of site for Report  on the basis of User Access of  Site on  09-May-07 by shiv 
                                              2:  changed  text from Manual to tabular on 29-Apr-08 by shiv  
...
function checkDate1()
{//@Vijay 23/Mar/2007 alert message edited 
...
function confirmData() {
	var flag = checkDate1();
	if (flag) {
		document.frm.action="T_DestinationReportPost.jsp";
		openWaitingProcess();
		frm.submit();
	}
}
...
<table width="100%" align="center" border="0" cellpadding="5" cellspacing="1">
  <tr>
	...
	<ul class="pagebullet">
      <li class="pageHead"><%=dbLabelBean.getLabel("label.report.starsreportdestinationwisereport",strsesLanguage) %></li>
    </ul></td>	
  </tr>
  </table>
<%
...
<form name="frm" method=post action="T_DestinationReportPost.jsp"> 
...
<%
		...
		while(itr.hasNext())
         ...
         //Added by 09-May-07 by shiv close	 
...
<tr class="formtr2" style="display: none;">
<td class="formtr2"><%=dbLabelBean.getLabel("label.report.viewreportinformat",strsesLanguage) %> </td>
<td class="formtr1">
<input type="radio" name="selectRadioType" value="Pie-Chart"> <%=dbLabelBean.getLabel("label.report.graphical",strsesLanguage) %>
<input type="radio" name="selectRadioType" value="Manual" checked="checked"><%=dbLabelBean.getLabel("label.report.tabular",strsesLanguage) %> <!-- changed  text from Manual to tabular on 29-Apr-08 by shiv   -->
</td>
</td>
</tr>
<%-- End --%>
