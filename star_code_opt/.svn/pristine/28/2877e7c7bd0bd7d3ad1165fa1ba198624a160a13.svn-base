...
function showData(){
	...
	if(checkval=='2')
	...
	else if(checkval=='1')
		{
				...
				{
					//window.location.href='T_TravelRequisitionWorkFlow.jsp?chkval='+checkval+'&fromflag='+fromval+'&toflag='+toval;
					openWaitingProcess();
					return true;
				}
				else
...
//added by manoj chand on 30 june 2011 to implement export to excel
function write1(loadflag)
{
	var info='';
	var checkflag=document.getElementById("chkData").value;
	if(checkflag!=1){
		info='<font size=2>'+'<%=dbLabelBean.getLabel("label.checkstatus.internationalrequisitionsfrom",strsesLanguage)%>'+ '<b>'+ document.frm.from.value +'</b>'+'<%=dbLabelBean.getLabel("label.approverequest.to",strsesLanguage)%>' +'<b>'+ document.frm.to.value+'</b></font>';
	}else{
		info='<b>'+'<%=dbLabelBean.getLabel("label.checkstatus.allinternationalrequisitions",strsesLanguage)%>'+'</b>';
	}
...
//added by manoj on 04 july 2011 to show processing image
...
function childwindowSubmit(){
	document.frm.action='T_TravelRequisitionWorkFlow.jsp';
	document.frm.submit();
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<ul class="pagebullet">
	<%if(ssflage.equals("3")){ %>
      <li class="pageHead"> <%=dbLabelBean.getLabel("label.checkstatus.interconttravelrequisitionsworkflow",strsesLanguage)%></li>
      <%}else{ %>
      <li class="pageHead"> <%=dbLabelBean.getLabel("label.checkstatus.internationaltravelrequisitionsworkflow",strsesLanguage)%></li>
      <%} %>
    </ul></td>
    <td height="40" align="right" valign="bottom" class="bodyline-top"  ><table align="right" border="0" cellspacing="0" cellpadding="0">
      <tr align="right">
      <td>
     <ul id="list-nav">
...
<form method=post name=frm action="T_TravelRequisitionWorkFlow.jsp" style="display: inline; margin: 0; " onSubmit="return showData();">  
  ...
  <input type=hidden name=va>
   ...
   <tr> 
     <td class="formtr3" align=center colspan="14">
       ...
       &nbsp;&nbsp;<input type=submit name="Gobutton" id="go" value=" <%=dbLabelBean.getLabel("button.search.go",strsesLanguage) %> " class=formbutton onclick="document.frm.action='T_TravelRequisitionWorkFlow.jsp';">
...
<span id=main>
  <tr class="formhead"> 
	...
	<td class="listhead" width="10%" align="center"><%=dbLabelBean.getLabel("label.global.currency",strsesLanguage)%></td>
...
if(strCheckType.equalsIgnoreCase("1"))
{
	strSql="SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID, TD.TRAVEL_REQ_NO, ISNULL(RTRIM(.DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER,ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATE,ISNULL(RTRIM(.DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,ISNULL(RTRIM(.DBO.PRESENTAPPROVER(TD.TRAVEL_ID,'I',TA.TRAVELLER_ID)),'NA') AS NEXTWITH,ISNULL(RTRIM(.DBO.WORKFLOWSTATUS(TD.TRAVEL_ID,'I')),'NA') AS NEXTWITH, DBO.REQUISITIONATTACHMENT(TD.TRAVEL_ID,'I')AS ATTACHMENT, DBO.TRAVEL_COMMENT(TD.travel_id,'I') AS COMMENT,td.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG,"
			...
			+" FROM T_APPROVERS TA INNER JOIN T_TRAVEL_STATUS TS ON TA.TRAVEL_ID=TS.TRAVEL_ID INNER JOIN T_TRAVEL_DETAIL_INT TD ON TD.TRAVEL_ID=TS.TRAVEL_ID INNER JOIN M_SITE SM ON SM.SITE_ID=TD.SITE_ID LEFT OUTER JOIN M_COMPANY CM ON SM.COMPANY_ID=CM.CID WHERE TS.TRAVEL_STATUS_ID !=1 AND TS.TRAVEL_TYPE='I' AND TA.TRAVEL_TYPE='I' AND  TD.C_USERID  = "+Suser_id+" AND TD.STATUS_ID=10 AND TS.STATUS_ID=10";
}else{
...
//rs = dbConBean.executeQuery(strSql);
//changed by manoj on 06 july to scroll resultset
...
%>
  ...
  <tr VALIGN="TOP" class="<%=strStyleCls%>"> 
    <td class="listhead" width="3%" align="Left"><%=++intSerialNo%><br> <a href="#" title="<%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%>" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=I&whichPage=T_TravelRequisitionWorkFlow.jsp&targetFrame=yes','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strImagePrint%></a> <a href="#" onClick="MM_openBrWindow('requisitionAttachment.jsp?purchaseReqID=<%=strTravelId%>&err=0&travel_type=I&whichPage=T_TravelRequisitionWorkFlow.jsp&targetFrame=yes','Attachments','scrollbars=yes,resizable=yes,width=875,height=550')" title="<%=dbLabelBean.getLabel("label.global.addattachments",strsesLanguage)%>"><%=strAttachment%></a> </td>	
   <td class="listdata" width="5%" align="center"><a href="#" onClick="MM_openBrWindow('<%=strPageUrl%>?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>&travel_type=I&travelType=I','SEARCH','scrollbars=yes,resizable=yes,width=975,height=600')"><%=strTravelRequestNo%></a> </td>
   ...
   <!--<td class="listdata" width="5%" align="center"><%//=strDivName%>&nbsp;<b>/</b>&nbsp;<%//=strSiteName%></td>-->
	...
	<td class="listdata" width="13%" align="center"><B><%=strNextApprover%></B></td>
    	<td class="listdata" width="13%" align="center"><a href="#" onClick="MM_openBrWindow('T_TravelRequisitionWorkFlowDiagram.jsp?travelRequisitionId=<%=strTravelId%>&strTravellerId=<%=strTravellerId%>&strTravelRequestNo=<%=strTravelRequestNo%>&travelType=I','SEARCH','scrollbars=yes,resizable=yes,width=775,height=250')"><%=strWorkflowStatus%></a></td>
    <td class="listhead" width="19%" align="center" nowrap="nowrap">
      <a href="#" onClick="MM_openBrWindow('requisitionAttachment.jsp?purchaseReqID=<%=strTravelId%>&err=0&travel_type=I&whichPage=T_TravelRequisitionWorkFlow.jsp&targetFrame=yes','Attachments','scrollbars=yes,resizable=yes,width=875,height=550')" title="<%=dbLabelBean.getLabel("label.global.addattachments",strsesLanguage)%>"><%=dbLabelBean.getLabel("link.approverequest.attach",strsesLanguage)%></a>  |<br> 
	...
	%>
	|<br><a href="#" onClick="fun_callwebservice('<%=strERPWS_URL %>','<%=strMATAWS_URL %>','<%=strCID %>','<%=strTravelRequestNo %>','<%=strTravelId %>','<%=strSiteId %>','I')">Cancel</a>
	<%}%>
	</td>
  </tr>
    <%
...
%>
   ...
   <tr align="center" class="formbottom"> 
    <td class="listhead" colspan="15" align="Left"><center>&nbsp;
	</center></td>
  </tr> 
  <tr>
