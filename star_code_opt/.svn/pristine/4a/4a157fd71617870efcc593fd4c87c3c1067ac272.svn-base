...
if(strUserAccessCheckFlag.equals("420")){
	...
	sSqlStr="SELECT TS.TRAVEL_STATUS_ID, ISNULL(TC.COMMENTS, '') AS COMMENTS FROM T_TRAVEL_STATUS TS LEFT OUTER JOIN TRAVEL_REQ_CANCEL TC ON TS.TRAVEL_ID=TC.TRAVEL_ID AND TS.TRAVEL_TYPE=TC.TRAVEL_TYPE WHERE TS.TRAVEL_TYPE='"+strTravelType+"' AND TS.TRAVEL_ID="+strRequisitionId ;
	rs = stmt.executeQuery(sSqlStr);
	
	if(rs.next()) {
		strTravelStatusId       = rs.getString("TRAVEL_STATUS_ID");
		strComments          	= rs.getString("COMMENTS");
	}
	rs.close();
	
	if("6".equals(strTravelStatusId)) {
		strDisableBtn    = "disabled";
		strPageHeadTitle = "Request "+dbLabelBean.getLabel("label.mail.hasbeencancelled",strsesLanguage);
	} else {
		strComments        = "";
		strDisableBtn    = "";
		strPageHeadTitle = dbLabelBean.getLabel("label.report.cancelrequest",strsesLanguage);
	}
%>
<script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<ul class="pagebullet">
      <li class="pageHead"><b><%= strPageHeadTitle%></b></li>
    </ul></td>
 ...
 <form name=frm action="T_Travel_Requisition_Cancel_ApprovedByAll_Post.jsp" method=post>
	...
	<table width="95%" align="center" border="0" cellpadding="5" cellspacing="1" class="formborder">
          <tr valign="middle"> 
            ...
            <td class="formtr1" colspan="6"> 
              <textarea name="comments" cols=107 rows=3 class="textBoxCss" <%=strDisableBtn %>><%=strComments %></textarea>
            </td>
          </tr>
          <tr> 
            <td colspan="7" class="formbottom" align=center> 
			  <input type=hidden name=buttonData>
              <input type="button" name="save" value=" <%=dbLabelBean.getLabel("label.requestdetails.cancelrequisition",strsesLanguage) %>" class="formbutton" onClick="CheckDataSave();" <%=strDisableBtn %> />
