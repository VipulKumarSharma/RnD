 ...
 *Modification 			:1 Display the mail Subject in different format 
								 ...
								 4:Code added for showing group travel  in case of domestic group travel_DOM  on shiv sharma on 19-Jun-08
...
//System.out.println("in here.."+strRequistionId);
...
//System.out.println("print the query.."+sSqlStr);
           	stmt= con.createStatement(); 
			rs = stmt.executeQuery(sSqlStr);
...
if(ReqTyp.equals("Domestic Travel"))
	...
	//code commented by manoj chand and replace with new query on 27 june 2013.	
			...
			rs2 = stmt2.executeQuery(strTravelMataMailSql);
			if(rs2.next()) {
				strMailID_TravelMata = rs2.getString("MATAGMBH");
			}
			rs2.close();
			stmt2.close();
		...
		//find the orignal approver name in case of handover mail  for sending mail added by shiv sharma on 25 sept 2008 
			...
			if(!"".equals(strMailID_OrignalApprover)){
				...
				strMailID_OrignalApprover=strMailID_TravelMata;
				   ...
				   //added by vijay on 09/04/2007 to change the mail subject 
				   
				   if(strGroupTravelFlag!=null  &&  (strGroupTravelFlag.trim().equals("Y"))) {
					    strMailSubject=dbLabelBean.getLabel("label.mail.starsnotification",strsesLanguage)+TType+" '"+strRequistionNumber.trim()+"' "+dbLabelBean.getLabel(groupGuestLabel,strsesLanguage)+" "+dbLabelBean.getLabel("label.mail.hasbeenrejected",strsesLanguage)+" ";//Mail Subject
				   } else {
						 strMailSubject=dbLabelBean.getLabel("label.mail.starsnotification",strsesLanguage)+TType+" '"+strRequistionNumber.trim()+"' "+dbLabelBean.getLabel("label.mail.hasbeenrejected",strsesLanguage)+" ";//Mail Subject
				   }
					...
					strMailMsg.append("<html><style>.formhead{ font-family:Arial;font-size:11px;font-style: normal;font-weight:normal;color:#000000;text-decoration:none;letter-spacing:normal;word-spacing:normal;border:1px #333333 solid;background:#E2E4D6;}</style><body bgcolor=\"#d0d0d0\">" + "\n");	
							...
							strMailMsg.append(dbLabelBean.getLabel("label.mail.dear",strsesLanguage)+" "+strRequistionCreatorName+",</font></p><p><font size=2 face=Verdana, Arial, Helvetica, sans-serif>"+dbLabelBean.getLabel("label.global.requisitionnumber",strsesLanguage)+" "+strRequistionNumber.trim()+" "+dbLabelBean.getLabel("label.mail.whichwasgeneratedbyyou",strsesLanguage)+" "+dbLabelBean.getLabel("label.approverrequest.from",strsesLanguage)+" "+strSiteName+" "+dbLabelBean.getLabel("label.mail.on",strsesLanguage)+" "+strRequistionCreatedDate+" "+dbLabelBean.getLabel("label.mail.hasbeenrejectedbyme",strsesLanguage)+"</p>"+ "\n"); 				
							strMailMsg.append("<u><font size=2 face=Verdana, Arial, Helvetica, sans-serif color=blue>"+dbLabelBean.getLabel("label.mail.requisitionsdetails",strsesLanguage)+":-</font></u><br><font size=2 face=Verdana, Arial, Helvetica, sans-serif >"+strGroupTravel+"<br> "+dbLabelBean.getLabel("label.mail.departuredateis",strsesLanguage)+" "+strTravelDate+".  </font>\n<br>"); 
									...
									try
										{
													...
													cstmt_mail.setString(13, "Signatory Rejects IT");
													cstmt_mail.execute();
													cstmt_mail.close();
										}
										catch(Exception b)
