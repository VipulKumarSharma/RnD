...
<%@page import="java.beans.DesignMode"%>
	...
	if(reportFlag.equalsIgnoreCase("false") && strUserAccessCheckFlag.equals("420")){	
		dbConBean.close();  
		dbConBean1.close();		
		securityUtilityBean.registerUnauthAccessLog(Suser_id, request.getRemoteAddr(), "Group_T_TravelRequisitionDetails_GmbH.jsp", "Unauthorized Access");
	...
	TravelRequest travelRequestObj = dao.getTravelRequestDetail(strTravelId, travelType, strTravellerId);
	//System.out.println("MAIl BODY : \n"+ new T_QuicktravelRequestDaoImpl().getGroupMailBodyForMataGmbH(strTravelId, travelType, strTravellerId));
	User travelerObj = travelRequestObj.getTraveller();
	User reqOriginatorObj = travelRequestObj.getOriginator();
	...
	<table width="99%" align="center" style="margin:0 auto;" border="0" cellpadding="5" cellspacing="1">
	  <tr>
	    <td>
		  <table width="100%" align="left" border="1" cellpadding="2" cellspacing="0" class="reporttble" bordercolor="#c7c7c5" style="border-collapse: collapse">
	        <tr>
	          <td height="0" colspan="6" align="left" class="reportHeading"><%=dbLabelBean.getLabel("label.requestdetails.basicinformationoforiginator",strsesLanguage) %></td>
	        </tr>
	        <tr>
	          <td width="12%" height="0" class="reportCaption" ><%=dbLabelBean.getLabel("label.global.requisitionnumber",strsesLanguage) %></td>
	          <td width="26%" height="0" class="reportdata"><%=travelRequestObj.getRequisitionNo()%></td>
	          <td width="12%" height="0" class="reportCaption" ><%=dbLabelBean.getLabel("label.requestdetails.originator",strsesLanguage) %></td>
	          <td width="20%" height="0" class="reportdata"><%=travelerObj.getFirstName().toUpperCase()%> <%= travelerObj.getLastName().toUpperCase()%></td>
	          <td width="14%" height="0" class="reportCaption" ><%=dbLabelBean.getLabel("label.approverrequest.originatedon",strsesLanguage) %></td>
	          <td width="16%" height="0" class="reportdata" ><%=travelRequestObj.getCreationDate()%></td>
	        </tr>
	        <tr>
	          <td width="12%" height="0" class="reportCaption" ><%=dbLabelBean.getLabel("label.requestdetails.division",strsesLanguage) %></td>
	          <td width="26%" height="0" class="reportdata" ><%=travelRequestObj.getDivisionName().toUpperCase()%></td>
	          <td width="12%" height="0" class="reportCaption" ><%=dbLabelBean.getLabel("label.requestdetails.unit",strsesLanguage) %></td>
	          <td width="20%" height="0" class="reportdata" ><%=travelerObj.getUnitName().toUpperCase()%></td>
	  ...
	  <tr>
        	<td>    		
        		<table width="100%" align="left" border="1" cellpadding="2" cellspacing="0" class="reporttble" bordercolor="#c7c7c5" style="border-collapse: collapse">
					...
					%>
          				<td height="0" colspan="8" class="reportHeading"><%=dbLabelBean.getLabel(groupGuestTravellerLabel,strsesLanguage) %></td>
        			</tr>
        			
        			<tr>
        				<td class="reportCaption" rowspan="<%=rowSpan%>"><%=dbLabelBean.getLabel("label.global.sno",strsesLanguage)%></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.global.firstname",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.global.lastname",strsesLanguage) %></td>        				
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.global.gender",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.global.contactno",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.global.frequentflyer",strsesLanguage) %></td>
        				<%if(travelType.equals("I")) { %>
        				<td class="reportCaption" width="18%" colspan="2"><%=dbLabelBean.getLabel("label.global.specialmeal",strsesLanguage) %></td>
        				<% } %>
        			</tr>
        			<%if(travelType.equals("I")) { %>
        			<tr>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.createrequest.passportnumber",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.user.nationality",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.createrequest.placeofissue",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.global.dateofissue",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.global.dateofexpiry",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.requestdetails.dob",strsesLanguage) %></td>
        				<td class="reportCaption" width="18%"><%=dbLabelBean.getLabel("label.global.visaexists",strsesLanguage) %></td>
        			</tr>
        			 <%
        			}
					  List groupTravelerList = travelRequestObj.getTravelerList();
					  Passport passportInfo = null;
					  User user = null;
					  if(groupTravelerList != null && !groupTravelerList.isEmpty()){
						  for(int i=0; i<groupTravelerList.size(); i++){
							  user = (User)groupTravelerList.get(i);
							  if(user != null){
							  	passportInfo = user.getPassport();
	  				%>
        			<tr>
        				<td class="reportCaption" width="5%" rowspan="<%=rowSpan%>"><%=i+1 %></td>
        				<td class="reportdata" width="18%"><%=user.getFirstName() %></td>
        				<td class="reportdata" width="18%"><%=user.getLastName() %></td>        				        				
        				<td class="reportdata" width="18%"><%=user.getGender() %></td>
        				<td class="reportdata" width="18%"><%=user.getContactNo() %></td>
        				<td class="reportdata" width="18%"><%=user.getFrequentFlyer() %></td>
        				<%if(travelType.equals("I")) { %>
        				<td class="reportdata" width="18%" colspan="2"><%=user.getMealPreferrence() %></td>
        				<% } %>
        			</tr>
        			<%if(travelType.equals("I")) { %>
        			<tr>
        				<td class="reportdata" width="18%"><%=passportInfo.getPassportNo() %></td>
        				<td class="reportdata" width="18%"><%=passportInfo.getNationality() %></td>
        				<td class="reportdata" width="18%"><%=passportInfo.getPlaceOfIssue() %></td>
        				<td class="reportdata" width="18%"><%=passportInfo.getDateOfIssue() %></td>
        				<td class="reportdata" width="18%"><%=passportInfo.getDateOfExpiry() %></td>
        				<td class="reportdata" width="18%"><%=user.getDateOfBirth() %></td>
        				<td class="reportdata" width="18%"><%=passportInfo.isVisaExists() ? "Yes" : "No" %></td>
        			</tr>
        			 <%
        				} 
        				}
	  ...
	  <% if(travelRequestObj.isFlightDetailExist()){ %>
        <tr id="flightDetailMainTR">
        	<td>
        		<table width="88%" align="left" border="1" cellpadding="2" cellspacing="0" class="reporttble" bordercolor="#c7c7c5" style="border-collapse: collapse">		        	
        			...
        			<%
        				  List flightList = travelRequestObj.getFlightDetailList();
	  ...
	  %>
	  <% if(travelRequestObj.isTrainDetailExist()) { %>
        <tr id="trnDetailTR1">
        	<td>
        		<table width="88%" align="left" border="1" cellpadding="2" cellspacing="0" class="reporttble" bordercolor="#c7c7c5" style="border-collapse: collapse">		        	
        			 <tr>
        			...
        			<%	  String isSpecialTrain = null;
        				  List trainList = travelRequestObj.getTrainDetailList();
        				  Train trainObj = null;
        				  Fare fareTrn = null;
        				  String journytypeStr = null;
        ...
        <tr>
		  ...
		  %>
        <%
        if(travelRequestObj.isRentA_CarDetailExist()){
        	List carList = travelRequestObj.getCarDetailList();
        ...
        <%if(travelRequestObj.isAccomodationDetailExist()){ 
        	List accommodationList = travelRequestObj.getAccommodationList();
        ...
        <%
        	Approver billingClient = travelRequestObj.getBillingApprover();
        %>
        <tr id="billingInstTR" style="display: block;">
        	<td>
        		<table width="100%" align="left" border="1" cellpadding="2" cellspacing="0" class="reporttble" bordercolor="#c7c7c5" style="border-collapse: collapse">
        			 <tr>
			          ...
			          <td height="0" class="reportdata" width="25%"><%=billingClient.getUnitName()%></td>
			          <td height="0" class="reportCaption" width="25%"><%=dbLabelBean.getLabel("label.requestdetails.billingapprover",strsesLanguage) %></td>
			          <td height="0" class="reportdata" width="25%"><%=billingClient.getName() %></td> 
			       </tr>
        		</table>
        	</td>
        </tr>
		...
		List approverLevelList = travelRequestObj.getApproverLevels();
	...
	<%
	  ...
	  List approverList = travelRequestObj.getApprovers();
							...
							<tr class="formhead1" height="5"> 
								<td   class="reportHeading" align="center">
								   <p    align=center><%=dbLabelBean.getLabel("label.requestdetails.clickonmailicontoviewmailflow",strsesLanguage) %>
								    <a href="#" onClick="MM_openBrWindow('T_travelRequisitionMails.jsp?travelReqNo=<%=travelRequestObj.getRequisitionNo()%>','REQMAILS','scrollbars=yes,resizable=yes,width=775,height=250')"><IMG SRC="images/EMAILIMG.gif?buildstamp=2_0_0" border=0 alt="<%= dbLabelBean.getLabel("label.requestdetails.viewemails",strsesLanguage)%>"></a>
								  </p>
								</td>
							</tr>
					 </table>
