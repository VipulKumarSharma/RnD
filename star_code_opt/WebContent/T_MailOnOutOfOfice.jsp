 ...
 *Description 			:This is the  jsp file  for inserting the mail information in req_mailbox table when user marks Out of Office  in STARS
 *Modification 			: 
 ...
 *Modification			: Correct the spelling of recipient in the mail body
 						: Resolve error coming in sql query at line 225
 *******************************************************/
%>
...
String strDelegateFrom					= request.getParameter("DelegateFromUserId");
...
String strComment						= request.getParameter("Comment").trim();
String strSite							= request.getParameter("site").trim();	
...
// query to check whether  loged user  is UH   
...
//System.out.println("strLAUHflage===="+strLAUHflage);
	...
	if 	(strTravelType.equals("A"))
		{
		strTravelType=" International & Domestic";
		}
		...
		if (strLAUHRole.equals("AD") || strLAUHRole.equals("UH-LA")) { // if  loged user is in (AD,UH,or LA) then following block will be executed  
				  if(strLAUHRole.equals("AD"))   	{
					       ...
					       strFromMail_id	="administrator.stars@mind-infotech.com";
							strRegardsName	=dbLabelBean.getLabel("label.mail.starsadmin",strsesLanguage);
	...
	*/
	...
	//change in query by manoj chand on 15 may 2012 to resolve error coming in that query
	//sSqlStr="set dateformat DMY  SELECT dbo.user_name("+strDelegateFrom+"),dbo.user_name("+strUserId+"),  convert(varchar(12),cast('"+strFromDate+"' as datetime(10)),13) as strFromDate,convert(varchar(12),cast('"+strToDate+"' as datetime(10)),13) as strToDate";
	sSqlStr="set dateformat DMY  SELECT dbo.user_name("+strDelegateFrom+"),dbo.user_name("+strUserId+"),  convert(varchar(12),cast('"+strFromDate+"' as datetime),13) as strFromDate,convert(varchar(12),cast('"+strToDate+"' as datetime),13) as strToDate"; 

	rs = dbConBean.executeQuery(sSqlStr);	
	if(rs.next())
		{
		strFromName 	=	rs.getString(1);	//Mail Reference Number
		strToName 		= 	rs.getString(2);	//Mail Reference Number
		strFromDate		=	rs.getString("strFromDate");	
		strToDate		=	rs.getString("strToDate");	
	}
	...
	if(rs.next())
	{
		...
		strMailSubject=dbLabelBean.getLabel("label.mail.outofofficeinformation",strLanguage);//Mail Subject
		...
		try
		{
			...
			strMailMsg.append("  <tr><td align=center bgcolor=#aa1220><font face=Verdana, Arial, Helvetica, sans-serif size=2><b><font color=#FFFFFF><font size=3>"+dbLabelBean.getLabel("label.mail.oooinformation",strLanguage)+"</font> </font></b></font></td></tr><tr><td bgcolor=#878787><font face=Verdana, Arial, Helvetica, sans-serif size=2><b></b></font></td></tr><tr><td><p><font color=#FFFFFF>.</font><font size=2 face=Verdana, Arial, Helvetica, sans-serif><br>"+ "\n"); 
			strMailMsg.append(" "+dbLabelBean.getLabel("label.mail.dear",strLanguage)+" "+strFullName+",</font></p><p><font size=2 face=Verdana, Arial, Helvetica, sans-serif>"+dbLabelBean.getLabel("label.mail.requisitionapprovalauthorityof",strLanguage)+" <b>"+strFromName+"</b> "+dbLabelBean.getLabel("label.mail.hasbeendelegatedtoyouforthe",strLanguage)+" <br><b> "+strTravelType+" "+dbLabelBean.getLabel("label.mail.requisitiontype",strLanguage)+"  <br>");
			strMailMsg.append("<br>"+dbLabelBean.getLabel("label.report.datefrom",strLanguage)+" <b>"+strFromDate+"</b> "+dbLabelBean.getLabel("label.approverequest.to",strLanguage)+" <b> "+strToDate+"</b>.<br>"+"\n"); 
			strMailMsg.append("<br><b>"+dbLabelBean.getLabel("label.mail.mycommentsareasfollow",strLanguage)+":-</b><br>"); 
			strMailMsg.append(strComment+".<br><br>"); 
			...
			try
			{
				...
				cstmt_mail.setString(2, "Out Of Office");		//Req_id
				cstmt_mail.setString(3, "Out Of Office");		//REQ_Number
				cstmt_mail.setString(4, strUserEmail);			//To
				cstmt_mail.setString(5, strFromMail_id);		//From
				cstmt_mail.setString(6, strCCMail_ID);			//CC   --remove by shiv on 28-Mar-08 
			...
			{
				System.out.println("ERROR in T_MailOnOutOfOffice.jsp---1--------"+b);		
			}
		}
		catch(Exception e)
		{
			System.out.println("ERROR in T_MailOnOutOfOffice.jsp---2--------"+e);		
		}
	}
	rs.close();
