...
*Modification 			: 1.Change the view of reports in case of MATA and Others
                          2.Added by shiv for showing Group Travel in case of group travel inspite of traveller Name on  02-Jul-08;     
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<ul class="pagebullet">
	<% if(ssflage.equals("3")){%>
      <li class="pageHead"> <b><%=dbLabelBean.getLabel("label.approverequest.domesticeuroperequisitionsapprovedbyme",strsesLanguage)%></b></li>
      <%}else{ %>
      <li class="pageHead"> <b><%=dbLabelBean.getLabel("label.approverequest.domesticrequisitionsapprovedbyme",strsesLanguage)%></b></li>
      <%} %>
    </ul></td>
...
<table width="100%" align="center" border="0" cellpadding="5" cellspacing="0"  class="formborder">  
 <form method=post name=frm action="MyApproved_D.jsp" onSubmit="return checkForm();">
...
rs.close();
 ...
 else
 {
	...
	/*if(strApproverRole!=null && (SuserRole.equals("CHAIRMAN") || SuserRole.equals("MATA") || SglobalApproverRole.equals("global")) )
    {*/
	  sSqlStr          =  "SET DATEFORMAT DMY SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(.DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(dbo.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(.DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,dbo.TRAVEL_COMMENT(TA.TRAVEL_ID,'D')AS TRAVEL_COMMENT,TD.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG,(select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_DOM TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_STATUS_ID<>1 AND TS.TRAVEL_TYPE='D' AND  TA.TRAVEL_TYPE='D'  AND TA.APPROVE_STATUS=10 AND TA.APPROVER_ID='"+Suser_id+"' AND CAST(FLOOR(CAST(APPROVE_DATE AS FLOAT))AS DATETIME) BETWEEN '"+strFrom+"' AND '"+strTo+"'";
	/*}
	else
	{
       sSqlStr          =  "SET DATEFORMAT DMY SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(.DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(dbo.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(.DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,dbo.TRAVEL_COMMENT(TA.TRAVEL_ID,'D')AS TRAVEL_COMMENT FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_DOM TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_STATUS_ID<>1 AND TS.TRAVEL_TYPE='D' AND  TA.TRAVEL_TYPE='D'  AND TA.APPROVE_STATUS=10 AND TA.APPROVER_ID='"+Suser_id+"' AND TA.SITE_ID="+strSiteIdSS+" AND CAST(FLOOR(CAST(APPROVE_DATE AS FLOAT))AS DATETIME) BETWEEN '"+strFrom+"' AND '"+strTo+"'";
	}*/	
 }
// stmt                           = con.createStatement(); 
 ...
 while(rs.next())
 {
								...
								{		
									strTraveller="Group/Guest Travel ";  
									  ...
									  if(SuserRole!=null && SuserRole.equals("MATA") )
                                           {
                                        urlPageMATA="Group_MATA_Report_DOM.jsp";
										   }
  							    }
								else
								  {
										...
										urlPageMATA="MATA_Report_DOM.jsp";
								}
	//============================close   
	...
	sSqlStr   ="SELECT  CASE WHEN COUNTALL=COUNTAPP THEN 'APPROVED BY ALL' WHEN " + 
				...
				" AND TRAVEL_STATUS_ID = 2 ORDER BY ORDER_ID ) AS PRESENTAPPROVER FROM   T_TRAVEL_DETAIL_DOM(NOLOCK)"+
...
%>
  <tr  valign="top" class="<%=strStyleCls%>"> 
    <td class="listhead" width="8%" align="Left">
    <a href="#" title="<%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%>" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=D&whichPage=MyApproved_D.jsp&targetFrame=no','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strImagePrint%></a><%=intSerialNo%></td>
	<!-- Vijay @26/Mar/2007 -->
     <%// Vijay 05/04/2007 Change the logic in case of MATA
    if(SuserRole!=null && SuserRole.equals("MATA") )
    {
%> 
	<td class="listdata" width="13%" align="center"><a href="#" onClick="MM_openBrWindow('<%=urlPageMATA%>?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strTravelRequestNo%></A></td>
<%}
	else
	{%>
...
%>
	...
	<!--<td class="listdata" width="6%" align="center"><%//=strDivName%>&nbsp;<b>/</b>&nbsp;<%//=strSiteName%></td>-->
	 <td class="listhead" width="20%" align="CENTER" nowrap="nowrap">  
        <a href="#" title="<%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%>" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=D&whichPage=MyApproved_D.jsp&targetFrame=no','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%></a> <br> <a href="#" title="<%=dbLabelBean.getLabel("label.approverequest.history",strsesLanguage)%> " onClick="MM_openBrWindow('UnderConstruction.html','HELP','scrollbars=yes,resizable=no,width=600,height=316')"><%=dbLabelBean.getLabel("label.approverequest.history",strsesLanguage)%></a>  </td>
  </tr>
    <%
  intSerialNo++;
}
