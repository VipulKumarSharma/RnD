...
<%
	 ...
	 *Description 		     	 :This file is to create international group/guest intemediate journey
	 *Editor					 :Eclipse 3.5
...
request.setCharacterEncoding("UTF-8");
	...
	if (strAcualTravelId != null && !strAcualTravelId.equals("null")
			&& !strAcualTravelId.equals("")
			&& !strAcualTravelId.equals("new")) {
		strUpdateFlag = "yes";
	} else
		strUpdateFlag = "no";

	strDCity = request.getParameter("strDeptCity") == null
			? ""
			: request.getParameter("strDeptCity");
	strACity = request.getParameter("strArrCity") == null
			? ""
			: request.getParameter("strArrCity");
	strDDate = request.getParameter("strDeptDate") == null
			? ""
			: request.getParameter("strDeptDate");
	strAirline = request.getParameter("strAirlineName") == null
			? ""
			: request.getParameter("strAirlineName");
	strMode = request.getParameter("strMode") == null ? "1" : request
			.getParameter("strMode");

	//New Change Start
	strVisaRequired = request.getParameter("visa") == null
			? "1"
			: request.getParameter("visa");;
	strVisaComment = request.getParameter("visaComment") == null
			? ""
			: request.getParameter("visaComment");
	//New Change End
	//added new for preffered time for immediate destination on 25-Oct-07 by shiv  
	strPreferTimeIntermediate = request
			.getParameter("preferTimeInterMediate") == null
			? "2"
			: request.getParameter("preferTimeInterMediate");
	strInterimId = request.getParameter("interimTravelId") == null
			? ""
			: request.getParameter("interimTravelId");
	...
	String strdepCity = request.getParameter("depCity") == null
			? ""
			: request.getParameter("depCity");
	String strarrCity = request.getParameter("arrCity") == null
			? ""
			: request.getParameter("arrCity");
	String strdepDate = request.getParameter("depDate") == null
			? ""
			: request.getParameter("depDate");
	String strprfTime = request.getParameter("preferTime") == null
			? ""
			: request.getParameter("preferTime");
	String strairName = request.getParameter("nameOfAirline") == null
			? ""
			: request.getParameter("nameOfAirline");
	String strtrvClass = request.getParameter("travelClass") == null
			? ""
			: request.getParameter("travelClass");
	String strsetPrf = request.getParameter("seatpreference") == null
			? ""
			: request.getParameter("seatpreference");
	String strHotel = request.getParameter("hotel") == null
			? "0"
			: request.getParameter("hotel");
	String strCur = request.getParameter("currency") == null
			? "USD"
			: request.getParameter("currency");
	String strPlace = request.getParameter("place") == null
			? ""
			: request.getParameter("place");
	String strBudget = request.getParameter("budget") == null
			? ""
			: request.getParameter("budget");
	String strCheckin = request.getParameter("checkin") == null
			? ""
			: request.getParameter("checkin");
	String strCheckout = request.getParameter("checkout") == null
			? ""
			: request.getParameter("checkout");
	String strOthcomment = request.getParameter("otherComment") == null
			? ""
			: request.getParameter("otherComment");
	String strUserids = request.getParameter("userids") == null
			? ""
			: request.getParameter("userids");

	String strurlflag = "depCity=" + strdepCity + "&arrCity="
			+ strarrCity + "&depDate=" + strdepDate + "&preferTime="
			+ strprfTime + "&nameOfAirline=" + strairName
			+ "&travelClass=" + strtrvClass + "&seatpreference="
			+ strsetPrf + "&hotel=" + strHotel + "&currency=" + strCur
			+ "&place=" + strPlace + "&budget=" + strBudget
			+ "&checkin=" + strCheckin + "&checkout=" + strCheckout
			+ "&otherComment=" + strOthcomment + "&userids="
			+ strUserids;
	//strurlflag = URLEncoder.encode(strurlflag,"UTF-8");
	//strurlflag=request.getParameter("urlflag")==null?strurlflag:request.getParameter("urlflag");
	//System.out.println("strurlflag----gip--->"+strurlflag);
%>
...
<script language="JavaScript">
		...
		function checkData(f1) {
			...
			if(document.frm.visa[1].checked)
			{
			   if(document.frm.visaComment.value=="")
				...
				var flag = checkDate(f1,todayDate,depDate,f1.todayDate,f1.strDeptDate,
				'<%=dbLabelBean.getLabel("alert.global.departuredatecannotsmallerthantodaydate",strsesLanguage)%>',
				...
				{
					flag = checkDate(f1,openerPageDepDate,depDate,f1.strDeptDate,f1.strDeptDate,
					'<%=dbLabelBean.getLabel("message.createrequest.group.intermediatedeparturedatecannotbesmaller",strsesLanguage)%>',
					'<%=dbLabelBean.getLabel("label.global.no", strsesLanguage)%>');
					if(flag == false) {
					  return flag;
					}
	...
	function closeWindow(){
		  if(document.frm.strDeptCity.value!='' || document.frm.strArrCity.value!='' || document.frm.strDeptDate.value!=''  || document.frm.strAirlineName.value!='')
			...
			//window.opener.frm.action = "Group_T_IntTrv_Details.jsp";
			//window.opener.location.href = "Group_T_IntTrv_Details.jsp?groupflag=Y&depCity=<%=strdepCity%>&arrCity=<%=strarrCity%>&depDate=<%=strdepDate%>&preferTime=<%=strprfTime%>&nameOfAirline=<%=strairName%>&travelClass=<%=strtrvClass%>&seatpreference=<%=strsetPrf%>&hotel=<%=strHotel%>&currency=<%=strCur%>&place=<%=strPlace%>&budget=<%=strBudget%>&checkin=<%=strCheckin%>&checkout=<%=strCheckout%>&otherComment=<%=strOthcomment%>";
			//alert("IN sds");
		//Commented to not refresh parent page
			//window.opener.location.href = "Group_T_IntTrv_Details.jsp?groupflag=Y&travelId=<%=strAcualTravelId%>&<%=strurlflag.trim()%>";
...
<form name="frm" method="post"
	action="InternationalInterimJourney_Post.jsp?GroupFlag=yes&<%=strurlflag%>">
<table width="100%" border="0" cellspacing="0" cellpadding="8">
	<tr>
		<td width="77%" class="bodyline-top">
		<ul class="pagebullet">
			<li class="pageHead"><%=dbLabelBean.getLabel("label.global.internationalgroupintermediatedestinations",strsesLanguage)%></li>
		</ul>
		</td>
	</tr>
</table>
...
<!--<tr>
 	<td width="14" height="10" background="images/index_04.png?buildstamp=2_0_0"><img
		src="images/index_04.png?buildstamp=2_0_0" width="14" height="10" /></td>
	<td height="10" colspan="2" background="images/index_05.png?buildstamp=2_0_0">&nbsp;</td>
	<td width="3" height="10" background="images/index_06.png?buildstamp=2_0_0"></td>
</tr> -->
<tr>
	<td height="10">&nbsp;</td>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td height="10" align="left" background="images/headerBG.png?buildstamp=2_0_0"><img
				src="images/formTitIcon.png?buildstamp=2_0_0" width="30" height="29"
				align="absmiddle" /> <span class="formTit"><%=dbLabelBean.getLabel("label.global.intermediatedestinations",strsesLanguage)%></span></td>
			<td align="left" background="images/headerBG.png?buildstamp=2_0_0"><img
				src="images/help.png?buildstamp=2_0_0" width="44" height="16" border="0"
				align="right"
				onClick="MM_openBrWin('helpintrimjourneyinternational.html#100a','','scrollbars=YES,resizable=no,location=no,directories=no,toolbar=no,menubar=no,left=10,top=10,width=800,height=350,status=no,location=no')" /></td>
			<br>
				...
				<tr>
					...
					<table width="98%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							...
							<td width="28%" height="20" align="left" valign="bottom"
								class="label1"><%=dbLabelBean.getLabel("label.global.departurecity",strsesLanguage)%></td>
							<td width="28%" height="20" align="left" valign="bottom"
								class="label1">&nbsp;&nbsp;<%=dbLabelBean.getLabel("label.global.arrivalcity",strsesLanguage)%></td>
						...
						<tr>
							...
							<td height="25" align="left" valign="top"><input
								name="strDeptCity" type="text" class="textBoxCss" size="22"
								value="<%=strDCity%>"
								onKeyUp="return test1('strDeptCity', 30, 'c')" maxlength="30" />
							</td>
							<td align="left" valign="top"><span class="label2">
							&nbsp;&nbsp;<input name="strArrCity" type="text"
								class="textBoxCss" size="22" value="<%=strACity%>"
								onKeyUp="return test1('strArrCity', 30, 'c')" maxlength="30" /></span>
							</td>
							<td align="left" valign="top"><input name="strDeptDate"
								type="text" class="textBoxCss" id="test2" size="7"
							...
							<a
								href="javascript:show_calendar('frm.strDeptDate','a','a','DD/MM/YYYY');"
								onmouseover="window.status='Date Picker';return true;"
								onmouseout="window.status='';return true;"><img
								src="images/calender.png?buildstamp=2_0_0" name="imageField" border="0"
								align="absmiddle" id="imageField" /></a> <script
						...
						<tr>
							...
							<td height="25" align="left" valign="top" class="label2"><input
								name="strAirlineName" type="text" class="textBoxCss" size="22"
								value="<%=strAirline%>"
								onKeyUp="return test1('strAirlineName', 16, 'cn')"
								maxlength="30" /></td>
							<td height="25" align="left" valign="top" class="label2">&nbsp;&nbsp;<select
								name="strClass" class="textBoxCss">
								<%
									if (strMode.equals("1"))
							...
							<td height="25" align="left" valign="top" class="label2"><select
								name="seatPreffered" class="textBoxCss">
								<%
									strSql = "SELECT  seat_id, Seat_Name FROM  M_SEAT_PREFER WHERE   (Mode_id = 1) AND (TRAVEL_AGENCY_ID = 1) AND (STATUS_ID = 10) ORDER BY SEAT_ID";
									rs = dbConBean.executeQuery(strSql);
									while (rs.next()) {
								%>
								<option value="<%=rs.getString(1)%>"><%=rs.getString(2)%></option>
						...
						<tr>
							<td height="12"></td>
							<td height="12"></td>
							<td height="12" align="left" valign="bottom"><span
								class="label2">&nbsp;&nbsp;<%=dbLabelBean.getLabel("label.global.visacomments",strsesLanguage)%></span></td>
						</tr>
						<tr>
							<td align="left" valign="top" class="label2"><!--VISA REQUIRED--></td>
							<td valign="top" align="left" class="label2"><input
								name="visa" type="radio" value="1" checked="checked"
								onClick="visaOnClick(this.form)" /> <%=dbLabelBean.getLabel("label.global.yes", strsesLanguage)%>
							<input name="visa" type="radio" value="2"
								onClick="visaOnClick(this.form)" /><%=dbLabelBean.getLabel("label.global.no", strsesLanguage)%>
							</td>
							<td height="40" align="left" valign="top" class="label2">&nbsp;&nbsp;<textarea
								name="visaComment" disabled
								onKeyUp="return test1('visaComment', 100, 'all')"
								maxlength="200"><%=strVisaComment%></textarea></td>
							<script language="javascript">
						...
						<tr align="center">
							<td height="25" colspan="6" align="center" class="label2"><input
								name="add" type="submit" class="formbutton"
								value=" <%=dbLabelBean.getLabel("label.global.add", strsesLanguage)%>"
								onclick="return checkData(this.form);" /></td>
						</tr>
					</table>
					</td>
				</tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td height="29" align="left" background="images/headerBG.png?buildstamp=2_0_0"><img
				src="images/formTitIcon.png?buildstamp=2_0_0" width="30" height="29"
				align="absmiddle" /><span class="formTit"><%=dbLabelBean.getLabel("label.global.intermediatedestinationsdetails",strsesLanguage)%></td>
			<td background="images/index_03.png?buildstamp=2_0_0"></td>
		</tr>
		<tr>
			<td width="14" background="images/index_01.png?buildstamp=2_0_0"></td>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<table width="100%" border="0" cellspacing="1" cellpadding="3">
						<tr class="formtr2">
							...
							<td class="formhead"><%=dbLabelBean.getLabel("label.global.class", strsesLanguage)%>
							</td>
							<td class="formhead"><%=dbLabelBean.getLabel("label.global.visarequired",strsesLanguage)%></td>
							<td class="formhead"><%=dbLabelBean.getLabel("label.global.visacomments",strsesLanguage)%>
							</td>
						...
						//query changed by manoj chand on 03 dec 2012 to display intermediate details in ordering
							...
							{
								strSql = "SELECT TRAVEL_FROM, TRAVEL_TO,ISNULL(DBO.CONVERTDATEDMY1(TRAVEL_DATE),'-') AS TRAVEL_DATE,DBO.TIMENAME(TIMINGS)AS TIMENAME,seat_name as SEAT_PREFFERED,.DBO.TRAVEL_MODE_NAME(TRAVEL_MODE) AS TRAVELMODENAME, MODE_NAME,.DBO.TRAVEL_CLASS_NAME(TRAVEL_MODE,TRAVEL_CLASS) AS TRAVELCLASSNAME,VISA_REQUIRED,VISA_COMMENT,JOURNEY_ORDER,DBO.TIMENAME(TIMINGS) as TimingsName FROM T_JOURNEY_DETAILS_INT   INNER JOIN  M_SEAT_PREFER ON  T_JOURNEY_DETAILS_INT.SEAT_PREFFERED = M_SEAT_PREFER.seat_id WHERE JOURNEY_ORDER>1 AND  TRAVEL_ID='"
										+ strAcualTravelId
										+ "' AND T_JOURNEY_DETAILS_INT.STATUS_ID=10 AND T_JOURNEY_DETAILS_INT.APPLICATION_ID = 1 ORDER BY year(travel_date),month(travel_date),day(travel_date)";
								/*
								...
								strSql = "SELECT TRAVEL_FROM, TRAVEL_TO,TRAVEL_DATE,DBO.TIMENAME(TIMINGS)AS TIMENAME,seat_name as SEAT_PREFFERED,.DBO.TRAVEL_MODE_NAME(TRAVEL_MODE) AS TRAVELMODENAME, MODE_NAME,.DBO.TRAVEL_CLASS_NAME(TRAVEL_MODE,TRAVEL_CLASS) AS TRAVELCLASSNAME,VISA_REQUIRED,VISA_COMMENT, INTERIM_ID,PARENT_ID FROM T_INTERIM_JOURNEY  INNER JOIN  M_SEAT_PREFER ON  T_INTERIM_JOURNEY.SEAT_PREFFERED = M_SEAT_PREFER.seat_id WHERE T_INTERIM_JOURNEY.PARENT_ID ='"
										+ strInterimId
										+ "' AND T_INTERIM_JOURNEY.APPLICATION_ID = 1  ORDER BY year(convert(datetime,TRAVEL_DATE,103)),month(convert(datetime,TRAVEL_DATE,103)),day(convert(datetime,TRAVEL_DATE,103))";

							}

							//System.out.println("strSql=======GROUP INTERIM=========="+strSql);
							String strVisaReq = "";
							String strVisaCom = "";
							int intCount = 1;
							rs = dbConBean.executeQuery(strSql);
						...
						<tr class="formtr2">
							...
							<td class="formtr1"><%=rs.getString("TIMENAME")%></td>
							<!-- <td class="formtr1"> </td>   -->
							...
							%>
							<td class="formtr1" onClick="return funConf();"><A
								HREF="T_DeleteInterim.jsp?actualTravelId=<%=strAcualTravelId%>&journeyOrder=<%=rs.getString("JOURNEY_ORDER")%>&travelType=I&groupFlag=GroupInt&<%=strurlflag%>"><%=dbLabelBean.getLabel("link.createeditlist.delete",strsesLanguage) %>
							</A></td>
							<%
								} else {
							%>
							<td class="formtr1" onClick="return funConf();"><A
								HREF="T_DeleteInterim.jsp?interimID=<%=rs.getString("INTERIM_ID")%>&travelID=<%=rs.getString("PARENT_ID")%>&travelType=I&groupFlag=GroupInt&<%=strurlflag%>"><%=dbLabelBean.getLabel("link.createeditlist.delete",strsesLanguage) %>
							</A></td>
							<%
								}
							%>
				...
				<tr>
					<td align="center" bgcolor="#FFFFFF" class="newformbot"><span
						class="label2"> <input name="add2" type="button"
						class="formbutton"
						value="<%=dbLabelBean.getLabel("button.global.saveandclose",strsesLanguage)%>"
						onclick="closeWindow();" /> </span></td>
				</tr>
			</table>
			</td>
			<td width="11" background="images/index_03.png?buildstamp=2_0_0"></td>
...
<INPUT type="hidden" name="travel_id" value="<%=strInterimId%>" /> <input
	type="hidden" name="todayDate" value="<%=strCurrentDate%>" /> <!--  HIDDEN FIELD  -->
<INPUT TYPE="hidden" name="interimTravelId" value="<%=strInterimId%>" />
<INPUT TYPE="hidden" name="actualTravelId"
	value="<%=strAcualTravelId%>" /></div>
</form>
</body>
