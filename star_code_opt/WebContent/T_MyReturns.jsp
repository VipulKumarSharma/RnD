 ...
 *Modification 			: 1.Added by shiv for showing Group Travel in case of group travel inspite of traveller Name on  21-Feb-08 ;  
 ...
 *Modification			:add function childwindowSubmit() to remove scripting error coming on close button of comment screen.
 
 *Modified by			:Manoj Chand
 *Date of Modification	:06 June 2012
 *Modification			:showing daterange default when page is open
 *******************************************************/
%>
...
//funtion added by manoj chand on 19 dec 2011 to remove scripting error coming on of close button of add comments screen.
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<ul class="pagebullet">
	<%if(ssflage.equals("3")){ %>
      <li class="pageHead"> <b><%=dbLabelBean.getLabel("label.approverequest.intercontrequisitionsreturnedbyme",strsesLanguage)%></b></li>
      <%}else{ %>
      <li class="pageHead"> <b><%=dbLabelBean.getLabel("label.approverequest.internationalrequisitionsreturnedbyme",strsesLanguage)%></b></li>
      <%} %>
    </ul></td>
...
rs.close();
  ...
  {    
     ...
     {*/
       sSqlStr=   "SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(.DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,DBO.TRAVEL_COMMENT(TA.TRAVEL_ID,'I')AS TRAVEL_COMMENT,  TD.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_TYPE='I' AND TA.TRAVEL_TYPE='I'  AND TA.APPROVE_STATUS=3 AND TA.APPROVER_ID='"+Suser_id+"' AND TS.TRAVEL_STATUS_ID=3";
	/* }
	 else
	 {
		 sSqlStr=   "SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,DBO.TRAVEL_COMMENT(TA.TRAVEL_ID,'I')AS TRAVEL_COMMENT FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_STATUS_ID=3 AND TS.TRAVEL_TYPE='I' AND TA.TRAVEL_TYPE='I' AND TA.APPROVE_STATUS=3 AND TA.APPROVER_ID='"+Suser_id+"'  AND TA.SITE_ID="+strSiteIdSS ;
	 }*/
 ...
 else
 {
	...
	/*if(SuserRole!=null && (SuserRole.equals("CHAIRMAN") || SglobalApproverRole.equals("global")) )
    {*/
	  sSqlStr          =  "SET DATEFORMAT DMY SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(.DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(.DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,DBO.TRAVEL_COMMENT(TA.TRAVEL_ID,'I')AS TRAVEL_COMMENT,TD.GROUP_TRAVEL_FLAG as GROUP_TRAVEL_FLAG, (select TRAVEL_AGENCY_ID from M_SITE where SITE_ID = td.SITE_ID) as travel_agency_id FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_STATUS_ID=3 AND TS.TRAVEL_TYPE='I' AND  TA.TRAVEL_TYPE='I' AND TA.APPROVE_STATUS=3 AND TA.APPROVER_ID='"+Suser_id+"' AND CAST(FLOOR(CAST(APPROVE_DATE AS FLOAT))AS DATETIME) BETWEEN '"+strFrom+"' AND '"+strTo+"'";
	/*}
	else
	{
       sSqlStr          =  "SET DATEFORMAT DMY SELECT DISTINCT TA.TRAVEL_ID, TA.TRAVELLER_ID,  TD.TRAVEL_REQ_NO, ISNULL(RTRIM(.DBO.USER_NAME(TA.TRAVELLER_ID)),'NA') AS TRAVELLER, ISNULL(DBO.CONVERTDATEDMY1(TA.C_DATETIME),'-') AS C_DATETIME, ISNULL(RTRIM(.DBO.USER_NAME(TA.C_USER_ID)),'NA') AS CREATED_BY,DBO.TRAVEL_COMMENT(TA.TRAVEL_ID,'I')AS TRAVEL_COMMENT FROM T_APPROVERS TA, T_TRAVEL_STATUS TS,T_TRAVEL_DETAIL_INT TD WHERE TA.TRAVEL_ID=TS.TRAVEL_ID AND TD.TRAVEL_ID=TS.TRAVEL_ID  AND TS.TRAVEL_STATUS_ID=3 AND TS.TRAVEL_TYPE='I' AND TA.TRAVEL_TYPE='I' AND TA.APPROVE_STATUS=3 AND TA.APPROVER_ID='"+Suser_id+"' AND TA.SITE_ID="+strSiteIdSS+" AND CAST(FLOOR(CAST(APPROVE_DATE AS FLOAT))AS DATETIME) BETWEEN '"+strFrom+"' AND '"+strTo+"'";
	}*/	
 }
...
%>
  <tr  valign="top" class="<%=strStyleCls%>"> 
    <td class="listhead" width="8%" align="Left"><a href="#" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=<%=strTravelType%>&whichPage=T_MyReturns.jsp&targetFrame=no','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=strImagePrint%></a><%=intSerialNo%></td>
    <td class="listdata" width="13%" align="center"><a href="#" 					onClick="MM_openBrWindow('<%=strPageUrl%>?purchaseRequisitionId=<%=strTravelId%>&traveller_Id=<%=strTravellerId%>&travelType=I','SEARCH','scrollbars=yes,resizable=yes,width=975,height=550')"><%=strTravelRequestNo%></A></td>
    ...
    <td class="listdata" width="14%" align="center"><b><%=strPresentApprover%><b></td>
	...
	<!--<td class="listdata" width="6%" align="center"><%//=strDivName%>&nbsp;<b>/</b>&nbsp;<%//=strSiteName%></td>-->
	 <td class="listhead" width="20%" align="CENTER" nowrap="nowrap">	  
        <a href="#" onClick="MM_openBrWindow('T_Travel_RequisitionComments.jsp?purchaseRequisitionId=<%=strTravelId%>&travel_type=<%=strTravelType%>&whichPage=T_MyReturns.jsp&targetFrame=no','SEARCH','scrollbars=yes,resizable=yes,width=775,height=550')"><%=dbLabelBean.getLabel("link.approverequest.comments",strsesLanguage)%></a> <br> <a href="#" onClick="MM_openBrWindow('UnderConstruction.html?purreq_id=<%=strTravelId%>','HELP','scrollbars=yes,resizable=yes,width=600,height=316')"> <%=dbLabelBean.getLabel("label.approverequest.history",strsesLanguage)%></a>
	</td>
  </tr>
    <%
  intSerialNo++;
