 ...
 *Author				:Manoj Chand
 *Date of Creation 		:23 September 2011
 *Copyright Notice 		:Copyright(C)2000 MIND.All rights reserved
 *Project	  			:STAR
 *Operating environment :Tomcat 6.0, sql server 2005
 *Description			:To take the domestic group intermediate journey details
 *Editor				:Eclipse
...
<%			request.setCharacterEncoding("UTF-8");
		 ...
		 // get the current date for checking the departureDate 
          ...
          ///Added prefered time  in intermmidiate  journey     ----by shiv sharma 25-Oct-07   
		   strPreferTimeIntermediate          =   request.getParameter("preferTimeInterMediate")==null?"2":request.getParameter("preferTimeInterMediate");
          ...
          String strurlflag="fwddepCity="+strfwddepCity+"&fwdarrCity="+strfwdarrCity+"&fwddepDate="+strfwddepDate+"&fwdpreferTime="+strfwdpreferTime+"&travelMode="+strtravelMode+"&fwdnameOfAirline="+strfwdnameOfAirline+"&fwdtravelClass="+strfwdtravelClass+"&fwdseatpreference="+strfwdseatpreference+"&fwdforTatkaalRequired="+strfwdforTatkaalRequired+"&fwdforCoupanRequired="+strfwdforCoupanRequired+"&fwdticketrefund="+strfwdticketrefund+"&transit="+strtransit+"&currency="+strcurrency+"&place="+strplace+"&budget="+strbudget+"&checkin="+strcheckin+"&checkout="+strcheckout+"&others="+strothers+"&userids="+struserids;
          
          //System.out.println("strurlflag--->"+strurlflag);
...
<script language="JavaScript">
		...
		function SubmitForm() {
			document.frm.action = "Group_InterimJourney_Dom.jsp";
			document.frm.submit();
		}
		...
		function checkData(f1) 
		{
			...
			if (document.frm.strAirlineName.value == ""){
				...
				var openerPageDepDate = window.opener.frm.fwddepDate.value;       //parent page departure date
...
/*	if(openerPageReturnDate != null && openerPageReturnDate != "")
	{
		var flag = checkDate(f1,depDate,openerPageReturnDate,f1.strDeptDate,f1.strDeptDate,'Intermediate Departure Date can not be grater than the Departure Date ','no');
		if(flag == false) {
		  return flag;
		}
	}*/
	...
	function closeWindow(frm){
         ...
         //comented by manoj chand on 15 june 2012 as there is no need to refresh parent page.
			//window.opener.setId("<%= strInterimId%>");  
			//window.opener.location.href = "Group_T_TravelDetail_Dom.jsp?domgroupflag=Y&travelId=<%=strAcualTravelId%>&<%=strurlflag.trim()%>";
			window.close();							
		}
...
<body> 
<form name="frm"  method="post" action="InterimJourney_Dom_Post.jsp?Groupflag=yes&<%=strurlflag %>">
<table width="100%" border="0" cellspacing="0" cellpadding="8">
  <tr>
    <td width="77%" class="bodyline-top">
	<ul class="pagebullet">
      <li class="pageHead"><%=dbLabelBean.getLabel("label.requestdetails.domesticgroupguestintermediatedestinations",strsesLanguage)%></li>
    </ul>
    </td>	
  </tr>
</table>
...
<table>    
   <!-- <tr>
      <td width="14" height="10" background="images/index_04.png?buildstamp=2_0_0"><img src="images/index_04.png?buildstamp=2_0_0" width="14" height="5" /></td>
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <tr>
                    ...
                    <td width="27%" height="20" align="left" valign="bottom" class="label1"><%=dbLabelBean.getLabel("label.global.departurecity",strsesLanguage)%></td>
                    <td width="33%" height="20" align="left" valign="bottom" class="label1"><%=dbLabelBean.getLabel("label.global.arrivalcity",strsesLanguage)%></td>
                ...
                <script language="JavaScript" type="text/javascript">
                  ...
                  <tr>
                    ...
                    <td height="20" align="left" valign="bottom" class="label1"><%=dbLabelBean.getLabel("label.global.mode",strsesLanguage) %></td>
                    <td height="20" align="left" valign="bottom" class="label1"><%=dbLabelBean.getLabel("label.global.preferredairlinetraincab",strsesLanguage) %></td>
                  ...
                  <tr>
                    <td height="25" colspan="6" align="center" valign="bottom" class="label2"><input name="add" type="submit" class="formbutton"  value="<%=dbLabelBean.getLabel("label.global.add",strsesLanguage) %>" onclick ="return checkData(this.form);"/></td>
                    <br>
</tr>
				...
				%>
					...
					if(strUpdateFlag.equals("yes"))
					{
						//System.out.println("if");
							%>
							<A HREF="T_DeleteInterim_Dom.jsp?actualTravelId=<%=strAcualTravelId%>&journeyOrder=<%=rs.getString("JOURNEY_ORDER")%>&travelType=D&Groupflag=GroupDom&<%=strurlflag %>"><%=dbLabelBean.getLabel("link.createeditlist.delete",strsesLanguage) %> </A>
            			    <%
					}
					else
					{
						//System.out.println("else");
							%>
				   			 <A HREF="T_DeleteInterim_Dom.jsp?interimID=<%= rs.getString("INTERIM_ID") %>&travelID=<%=rs.getString("PARENT_ID")%>&travelType=D&Groupflag=GroupDom&<%=strurlflag %>"> <%=dbLabelBean.getLabel("link.createeditlist.delete",strsesLanguage) %> </A>
							<%
					}
			%>
