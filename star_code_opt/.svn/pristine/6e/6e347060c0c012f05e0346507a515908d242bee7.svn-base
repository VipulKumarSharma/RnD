 ...
 *Description 			    :This is javascript file for generating  calender 
 *Editor				    :Eclipse
 *******************************************************/
	...
	function popUpCalendar1(ctl,	ctl2, format) {
		...
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
				...
				crossobj.top = fixedY==-1 ?	(ctl.offsetTop +	toppos + ctl.offsetHeight +	2)-210 :	fixedY
			...
			{
				hideCalendar()
				if (ctlNow!=ctl) {popUpCalendar1(ctl, ctl2, format)}
		}
