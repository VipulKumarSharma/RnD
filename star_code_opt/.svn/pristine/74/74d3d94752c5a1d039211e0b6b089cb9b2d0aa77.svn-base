...
String strSiteId ="", strUserId="", strMealId="", strSex="", strProjectNo="", strReasonForTravel="";
...
String strTotalFareCurrency="", strTotalFareAmount="";
//Approver Details
String strBillingClient = "", strBillingApprover = "", strShowflag = "n", strMailToMataFlag = "N";
String strVisaRequired = "", strVisaComment = "";
...
while (rs.next()) {
	...
	strSql = "select dbo.USER_NAME_MATA(" + strApproverid+ "),dbo.USER_NAME_MATA(dbo.finalooo(" + strApproverid+ ",getdate(),'i'))";
	rs7=null;
...
if(strTravelId==null) {
	...
	if(rsItnr.next()) {
			...
			strMealId            		= rsItnr.getString("MEAL");
			...
			strTotalFareCurrency     	= rsItnr.getString("FARE_CURRENCY") == null ? "" : rsItnr.getString("FARE_CURRENCY").trim();
			strTotalFareAmount       	= rsItnr.getString("FARE_AMOUNT")   == null ? "" : rsItnr.getString("FARE_AMOUNT").trim();
			
			if(strTravelType.equals("I")) {
				strVisaRequired      		= rsItnr.getString("VISA_REQUIRED");
				strVisaComment       		= rsItnr.getString("VISA_COMMENT");	
			}
		}
    rsItnr.close();
...
if(strVisaRequired == null) {// Remove default selection for VISA Required
	strVisaRequired = "";
}
...
String TRAVELLERSITEID="", SITENAME="", TRAVELLERUSERID="", FIRSTNAME="", MIDDLENAME="", LASTNAME ="", FULLNAMEPASSPORT="", FNAMEPASSPORT="", LNAMEPASSPORT="", DESIGNATION	= "", GENDER= "", EMAIL= "", CONTACT_NUMBER = "", DOB= "", PASSPORT_NO = "", NATIONALITY= "", PLACE_ISSUE = "", PP_ISSUE_COUNTRY_ID = "", DATE_ISSUE= "", EXPIRY_DATE= "", ADDRESS  = "", CURRENT_ADDRESS= "",  
		AIRLINE_NAME1= "", AIRLINE_NAME2= "", AIRLINE_NAME3= "", AIRLINE_NAME4= "", AIRLINE_NAME5= "", AIRLINE_NO1= "", AIRLINE_NO2= "", AIRLINE_NO3= "", AIRLINE_NO4= "", AIRLINE_NO5= "", HOTEL_NAME1= "", HOTEL_NAME2= "", HOTEL_NAME3= "", HOTEL_NAME4= "", HOTEL_NAME5= "", HOTEL_NO1= "", HOTEL_NO2= "", HOTEL_NO3= "", HOTEL_NO4= "", HOTEL_NO5= "", COST_CENTER= "0", COST_CENTER_NAME= "";	
			rs=null;
			...
			if(rs.next()) {
				...
				FULLNAMEPASSPORT	=  rs.getString("FULL_NAME");
			...
			//Get flag to show Total Travel Fare Block
			String strMailToMataFlagDB = dao.getMailToMataFlag(strSiteId);
			if(strMailToMataFlagDB!=null){
				strMailToMataFlag = strMailToMataFlagDB;
			}
...
<body>
	...
	<form name="frm" id="frmId" action="T_QuickTravel_Detail_GmbH_Post.jsp">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			if(strTravelId !=null && !strTravelId.equals("") && !strTravelId.equals("new")) {
			%>
				<td><ul class="pagebullet"><li><%=dbLabelBean.getLabel("label.editrequest.edittravelrequest",strsesLanguage)%>&nbsp;-&nbsp;<span style="font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 11px; font-weight: bold;color: #0000ff;">[<%=strTravelReqNo %>]</span></li></ul></td>
			<%	} else {%>
				<td><ul class="pagebullet"><li><%=dbLabelBean.getLabel("label.createrequest.createtravelrequest",strsesLanguage)%></li></ul></td>
			<% } %>
			</tr>
			<tr><td class="hrSpace4Px"></td></tr>
		</table>
												...
												<td style="width:88%; text-align: left;">
													<table width="100%" border="0" cellspacing="0"
														...
														<%} else { %>
															...
															<td id="travelerNmLabel"><%=dbLabelBean.getLabel("label.global.travelername",strsesLanguage)%></td>
															<td id="travelerNm">
															<select name="userNameCombo" class="comboBoxCss" id="userNameCombo">
																<option value="-1" selected><%=dbLabelBean.getLabel("label.createrequest.travellername",strsesLanguage) %></option>
												              	<%
												              		...
												              		if(!strSiteId.equals("0")) {
												              			if(SuserRoleOther.equals("LA") || SuserRoleOther.equals("TC")) {
															...
															<script language="javascript">
												                document.frm.userNameCombo.value= "<%=strUserId%>";
									            			</script>
															</td>													
														</tr>
													</table>
												...
												<tr>
													<td id="left">
														<table width="100%" border="0" cellspacing="0" cellpadding="0">
															<tr>
																<td>
																	<table class="personalDetailHeaderTbl">																		
																		<tr>
																			<td class="perDetl"><%=dbLabelBean.getLabel("label.global.personaladdress",strsesLanguage)%></td>																					
																		</tr>
																	</table>
																</td>
															</tr>
															<tr>
																...
																<table class="personalDetailInnerTbl">																	
																		<tr>
																			...
																			<td class="lastNamelabel"><%=dbLabelBean.getLabel("label.global.firstname",strsesLanguage)%>&nbsp;<%=dbLabelBean.getLabel("label.mylinks.asperpassport",strsesLanguage)%></td>
																		...
																		<tr>
																			...
																			<td class="contactNoLabel"><%=dbLabelBean.getLabel("label.global.contactno",strsesLanguage)%></td>
																			<td class="genderlabel"><%=dbLabelBean.getLabel("label.global.gender",strsesLanguage)%></td>
																		</tr>
																		<tr>
																			...
																			<td class="contactNoInput"><input type="text"
																				name="contactNo" class="textBoxCss"
																				value="<%=CONTACT_NUMBER%>"
																				id="contactNo" onKeyUp="return test2(this, 19, 'phone')" maxlength="20"></td>
																			<td class="genderInput">
																				<table class="personalDetailGenderTbl">
																					<tr>
																						<%if(GENDER!=null) {
																						if(GENDER.equals("Male")) {%>
																								...
																								<td class="genderInputlabel"><%=dbLabelBean.getLabel("label.global.male",strsesLanguage)%></td>
																								<td class="genderInputRadio">
																									<input name="sex" type="radio" value="2" id="femaleRd" disabled/>
																								</td>
																								<td class="genderInputlabel"><%=dbLabelBean.getLabel("label.global.female",strsesLanguage) %></td>
																								<% strSex= "1"; %>
																						<%} else {%>
																								<td class="genderInputRadio">
																		...
																		<tr>
																			...
																			<td id="specialMealLabel" class="mealPrefLabel"><%=dbLabelBean.getLabel("label.global.specialmeal",strsesLanguage) %></td>																														
																		</tr>
																		<tr>																			
																			...
																			<td id="specialMealSelectTd" class="mealPrefInput"><select name="specialMeal"
																				id="specialMeal" class="comboBoxCss">																				
																					<%Set entrySet = specialMeal.entrySet();
																					Iterator it = entrySet.iterator();
														                             while(it.hasNext())
														                             {
														                            	 Map.Entry entry = (Map.Entry)it.next();
														                            	 
														                            	 if(!strMealId.equals("") && entry.getKey().equals(strMealId)) {%>
															...
															<tr>
																<td>
																	<table class="rewardCardDetailMainTbl">
																		<tr>
																			<td>
																				<table class="rewardCardDetailInnerTbl">
																					<tr>
																						<td class="rewrdDett"><%=dbLabelBean.getLabel("label.global.rewardscarddetails",strsesLanguage) %></td>
																					</tr>
																				</table>
																			</td>
																		</tr>
																		<tr>
																			<td>
																				<table class="rewardCardDetailInner1Tbl">
																					<tr>
																						<td class="rewardCardDetailInner2Tbl">
																							<table class="rewardCardDetailInner3Tbl">
																								<tr>
																									<td class="airlineNameLabel"><b><%=dbLabelBean.getLabel("label.global.frequentflyer",strsesLanguage) %></b></td>
																								</tr>
																							</table>
																						</td>
																						<td class="rewardCardDetailInner4Tbl">
																							<table class="rewardCardDetailInner5Tbl">
																								<tr>
																									<td class="airlineNameLabel"><b><%=dbLabelBean.getLabel("label.global.hotelrewardcard",strsesLanguage) %></b></td>
																								</tr>
																							</table>
																						</td>
																					</tr>
																					<tr>
																						<td class="rewardCardDetailInner6Tbl">
																							<table class="rewardCardDetailInner7Tbl">
																								<tr>
																									<td class="airlineNameLabel"><%=dbLabelBean.getLabel("label.global.airlinename",strsesLanguage) %></td>
																									<td class="airlineNoLabel"><%=dbLabelBean.getLabel("label.requestdetails.number",strsesLanguage) %></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="airlineName1" class="textBoxCss" value="<%=AIRLINE_NAME1%>"
																										id="airlineName1" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="airlineNo1" class="textBoxCss" value="<%=AIRLINE_NO1%>"
																										id="airlineNo1" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																								<tr>
																									<td class="hrSpace4Px" colspan="2"></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="airlineName2" class="textBoxCss" value="<%=AIRLINE_NAME2%>"
																										id="airlineName2" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="airlineNo2" class="textBoxCss" value="<%=AIRLINE_NO2%>"
																										id="airlineNo2" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																								<tr>
																									<td class="hrSpace4Px" colspan="2"></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="airlineName3" class="textBoxCss" value="<%=AIRLINE_NAME3%>"
																										id="airlineName3" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="airlineNo3" class="textBoxCss" value="<%=AIRLINE_NO3%>"
																										id="airlineNo3" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																								<tr>
																									<td class="hrSpace4Px" colspan="2"></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="airlineName4" class="textBoxCss" value="<%=AIRLINE_NAME4%>"
																										id="airlineName4" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="airlineNo4" class="textBoxCss" value="<%=AIRLINE_NO4%>"
																										id="airlineNo4" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																								<tr>
																									<td class="hrSpace4Px" colspan="2"></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="airlineName5" class="textBoxCss" value="<%=AIRLINE_NAME5%>"
																										id="airlineName5" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="airlineNo5" class="textBoxCss" value="<%=AIRLINE_NO5%>"
																										id="airlineNo5" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																							</table>
																						</td>
																						<td class="rewardCardDetailInner8Tbl">
																							<table class="rewardCardDetailInner9Tbl">
																								<tr>
																									<td class="airlineNameLabel"><%=dbLabelBean.getLabel("label.global.hotelchain",strsesLanguage) %></td>
																									<td class="airlineNoLabel"><%=dbLabelBean.getLabel("label.requestdetails.number",strsesLanguage) %></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="hotelName1" class="textBoxCss" value="<%=HOTEL_NAME1%>"
																										id="hotelName1" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="hotelNo1" class="textBoxCss" value="<%=HOTEL_NO1%>"
																										id="hotelNo1" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																								<tr>
																									<td class="hrSpace4Px" colspan="2"></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="hotelName2" class="textBoxCss" value="<%=HOTEL_NAME2%>"
																										id="hotelName2" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="hotelNo2" class="textBoxCss" value="<%=HOTEL_NO2%>"
																										id="hotelNo2" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																								<tr>
																									<td class="hrSpace4Px" colspan="2"></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="hotelName3" class="textBoxCss" value="<%=HOTEL_NAME3%>"
																										id="hotelName3" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="hotelNo3" class="textBoxCss" value="<%=HOTEL_NO3%>"
																										id="hotelNo3" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																								<tr>
																									<td class="hrSpace4Px" colspan="2"></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="hotelName4" class="textBoxCss" value="<%=HOTEL_NAME4%>"
																										id="hotelName4" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="hotelNo4" class="textBoxCss" value="<%=HOTEL_NO4%>"
																										id="hotelNo4" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																								<tr>
																									<td class="hrSpace4Px" colspan="2"></td>
																								</tr>
																								<tr>
																									<td class="airlineNameInput"><input type="text"
																										name="hotelName5" class="textBoxCss" value="<%=HOTEL_NAME5%>"
																										id="hotelName5" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																									<td class="airlineNoInput"><input type="text"
																										name="hotelNo5" class="textBoxCss" value="<%=HOTEL_NO5%>"
																										id="hotelNo5" onKeyUp="return test2(this, 20, 'cn')" maxlength="20"></td>
																								</tr>
																							</table>
																						</td>
																					</tr>
													...
													<td id="right">
														<table width="100%" border="0" cellspacing="0" cellpadding="0">
																						...
																						<option  value='-1'><%=dbLabelBean.getLabel("label.createrequest.userfrombillingunit",strsesLanguage)%></option>
																							<%
																								...
																								strSql = "SELECT  1 "
																									...
																									if (!rs.next()) {
																										rs.close();
																										strSql = "select distinct userid, dbo.USER_NAME_MATA(userid) As UserName from M_userInfo "
																											+ "where (site_id="
																											+ strBillingClient
																										...
																										strSql = "SELECT distinct APPROVER_ID as userid, dbo.USER_NAME_MATA(APPROVER_ID) As UserName "
																											+ "FROM M_BILLING_APPROVER BA inner join M_userInfo UI on UI.userid = BA.approver_id "
																											+ "WHERE BA.SITE_ID= "
																											+ strBillingClient
																											+ " and status_id =10";
															...
															<tr><td class="hrSpace4Px"></td></tr>
															<tr>
																<td><div id="defaultApproverDiv" style="float:left; width:100%;"></div></td>
															</tr>																	
														</table>
													</td>
												</tr>
		...
		<% if("N".equalsIgnoreCase(strMailToMataFlag)) { %>				
			<!--Total Travel Fare Block Starts  -->
			<tr>
				<td class="journyType">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">											
						<tr>
							<td>
								<table width="50%" border="0" cellspacing="2" cellpadding="0">
									<tr>
										<td class="ttfLabel"><label class="requiredInput"><b><%=dbLabelBean.getLabel("label.createrequest.totaltravelfare",strsesLanguage)%></b></label></td>
										<!-- <td class="ttfLabel"></td> -->
										...
										<td class="ttfTotalFareBox">
											<input name="TotalFareAmount" id="totalfareamount" type="text" class="textBoxCss" onKeyUp="return test2(this, 10, 'n')" 
												value="<%=strTotalFareAmount %>" title="<%=dbLabelBean.getLabel("label.createrequest.totalfare",strsesLanguage)%>" />
										</td>
									</tr>
								</table>
							</td>
		...
		<%} %>	
			...
			<tr id="passportDtl" class="hide">
				<td>
					<table class="passportMainTbl">
						<tr>
							<td>
								<table class="passportHeaderTbl">
									<tr>
										<td class="perDetl"><%=dbLabelBean.getLabel("label.createrequest.passportinformation",strsesLanguage)%></td>
									</tr>
								</table>
							</td>
						</tr>
						<tr>
							<td class="journyInner">
								<table class="passportInnerTbl">
								<tr>
									<td class="completeNameslabel" colspan="3"><%=dbLabelBean.getLabel("label.global.completenameasperpass",strsesLanguage)%></td>
								</tr>
								<tr>									
									<td class="firstNameslabel"><%=dbLabelBean.getLabel("label.global.firstnameasperpass",strsesLanguage)%></td>
									<td class="lastNameslabel"><%=dbLabelBean.getLabel("label.global.lastnameasperpass",strsesLanguage)%></td>
									<td class="dobLabel"><%=dbLabelBean.getLabel("label.global.dob",strsesLanguage)%></td>
								</tr>
								<tr>									
									<td class="firstNamesInput"><input type="text"
										name="fNamePassport" class="textBoxCss" value="<%=FNAMEPASSPORT%>"
										id="fNamePassport" onKeyUp="return test2(this, 100, 'c')" maxlength="100"></td>
									<td class="lastNamesInput"><input type="text"
										name="lNamePassport" class="textBoxCss" value="<%=LNAMEPASSPORT%>"
										id="lNamePassport" onKeyUp="return test2(this, 100, 'c')" maxlength="100"></td>
									<td class="dobInput"><input type="text" name="dob"
													class="textBoxCss" onFocus="initializeDate(this);" value="<%=DOB%>" id="dob"></td>
								</tr>
								</table>
							</td>
						</tr>						
						<tr>
							<td class="journyInner">
								<table class="passportInnerTbl">
									<tr>
										<td class="passportNamelabel"><%=dbLabelBean.getLabel("label.global.passportnumber",strsesLanguage)%></td>
										...
										<td class="placeOfIssueLabel"><%=dbLabelBean.getLabel("label.createrequest.placeofissue",strsesLanguage)%></td>
										<td class="dateOfIssuelabel"><%=dbLabelBean.getLabel("label.global.dateofissue",strsesLanguage)%></td>
										<td class="dateOfExpiryLabel"><%=dbLabelBean.getLabel("label.global.dateofexpiry",strsesLanguage)%></td>
										<td class="visaExistsLabel"><%=dbLabelBean.getLabel("label.global.visaexists",strsesLanguage)%></td>
									</tr>
									<tr>
										<td class="passportNameInput"><input type="text" 
											name="passportNo" class="textBoxCss" value="<%=PASSPORT_NO%>" id="passportNo"
											 onKeyUp="return test2(this, 49, 'cn')" maxlength="49"></td>
										<td class="nationalityInput"><input type="text" 
											name="nationality" class="textBoxCss" value="<%=NATIONALITY%>" id="nationality"
										    onKeyUp="return test2(this, 29, 'c')" maxlength="20"></td>
										<td class="ppCountrySelect nationalityInput">
											<select name="pp_country" class="comboBoxCss" id="pp_country" title="<%=dbLabelBean.getLabel("label.user.country",strsesLanguage)%>">
												...
												<%	rs = dbConBean.executeQuery("SELECT COUNTRY_ID, COUNTRY_CODE, COUNTRY_NAME FROM M_COUNTRY WHERE STATUS_ID=10 AND APPLICATION_ID=1 ORDER BY COUNTRY_NAME");
													while(rs.next()) {
														if(PP_ISSUE_COUNTRY_ID != null && !"".equals(PP_ISSUE_COUNTRY_ID) && rs.getString("COUNTRY_ID").equals(PP_ISSUE_COUNTRY_ID)) {
												%>			<option value="<%=rs.getString("COUNTRY_ID")%>" selected="selected"><%=rs.getString("COUNTRY_NAME")%></option>
												<%		} else {
										...
										<td class="placeOfIssueInput"><input type="text"
											name="placeOfIssue" class="textBoxCss" value="<%=PLACE_ISSUE%>"
											id="placeOfIssue" onKeyUp="return test2(this, 99, 'c')" maxlength="99"></td>
										<td class="dateOfIssueInput"><input type="text"
											name="dateOfIssue" class="textBoxCss" onFocus="initializeDate(this);" value="<%=DATE_ISSUE%>" id="dateOfIssue"></td>
										<td class="dateOfExpiryInput"><input type="text"
											name="dateOfExpiry" class="textBoxCss" onFocus="initializeDate(this);" value="<%=EXPIRY_DATE%>"
											id="dateOfExpiry"></td>
										<td class="visaExistsInput">
											<table class="visaExistTbl" style="margin-top: -2px;">
												<tr valign="top">
													<td class="requiredRadioBoxDefault">
														<table width="100%" border="0" cellspacing="0"
															cellpadding="0" class="checkbox">
															<tr valign="top">
																<td class="labelTd"><label class="yes"
																	id="visaRequiredYes"> <input type="radio"
																		name="visa" value="1" id="visaRequired_Y"><%=dbLabelBean.getLabel("label.global.yes",strsesLanguage)%>
																</label></td>
																<td class="labelTd"><label class="no"
																	id="visaRequiredNo"> <input type="radio"
																		name="visa" value="2" 
																		id="visaRequired_N"><%=dbLabelBean.getLabel("label.global.no",strsesLanguage)%>
																</label></td>
															</tr>
														</table>
														<script language="javascript">
	    ...
	    <input type="hidden" name="showFlag" id="hidshowFlag"  value="<%=strShowflag %>" /> <!--  HIDDEN FIELD  -->
	    <input type="hidden" name="approverlist" value="<%=ApproverText%>"> <!--  HIDDEN FIELD  -->
	    <input type="hidden" name="sex" value= "<%=strSex%>" id="sex"/> <!--  HIDDEN FIELD  -->
	...
	<script type="text/javascript" src="ScriptLibrary/T_QuickTravelRequest_Gmbh.js?buildstamp=2_0_0"></script>
</body>
</html>
<script type="text/javascript">
...
$("select#siteNameCombo").live("change", function() {
    ...
    var urlParams=Params+"&reqpage="+reqpage+"&siteId="+siteId+"&ssflage="+ssflage+"&suserId="+suserId+"&suserRoleOther="+suserRoleOther;
    $.ajax({
           type: "get",
           ...
           success: function(result) {
           	if(result!=null) {
           		$("#userNameCombo").html('');
				$("#userNameCombo").get(0).options.add(new Option());
             	$("#userNameCombo").html(result.travellerHtml);             	
			}				
       	}
      });
...
$("select#userNameCombo").live("change", function() {
	var val = "";
	var workFlowCode = "0";
	var groupTravelFlag = "N";
	var userId = $("select#userNameCombo option:selected").val();
	var siteId = $("select#siteNameCombo option:selected").val();
	...
	var reqpage="quickTravelReqPersonalDetail";	  	
 	var Params='<%="language1="+strsesLanguage+"&suserRole="+SuserRole%>';
    var urlParams=Params+"&reqpage="+reqpage+"&userId="+userId+"&siteId="+siteId+"&travelType="+val+"&groupTravelFlag="+groupTravelFlag;
	$.ajax({
           type: "get",
           url: "QuickTravelRequestGMBHServlet",
           data: urlParams,
           ...
           success: function(result) {
			if(result!=null) {
				...
				$("#contactNo").val(result.CONTACT_NUMBER);
				$("#costCenter").val(result.COST_CENTER);
				$("#costCenterName").val(result.COST_CENTER_NAME);
				$("#contactNo").val(result.CONTACT_NUMBER);
				$("#airlineName1").val(result.AIRLINE_NAME1);
				$("#airlineName2").val(result.AIRLINE_NAME2);
				$("#airlineName3").val(result.AIRLINE_NAME3);
				$("#airlineName4").val(result.AIRLINE_NAME4);
				$("#airlineName5").val(result.AIRLINE_NAME5);
				$("#airlineNo1").val(result.AIRLINE_NO1);
				$("#airlineNo2").val(result.AIRLINE_NO2);
				$("#airlineNo3").val(result.AIRLINE_NO3);
				$("#airlineNo4").val(result.AIRLINE_NO4);
				$("#airlineNo5").val(result.AIRLINE_NO5);
				$("#hotelName1").val(result.HOTEL_NAME1);
				$("#hotelName2").val(result.HOTEL_NAME2);
				$("#hotelName3").val(result.HOTEL_NAME3);	
				$("#hotelName4").val(result.HOTEL_NAME4);
				$("#hotelName5").val(result.HOTEL_NAME5);
			    $("#hotelNo1").val(result.HOTEL_NO1);
				$("#hotelNo2").val(result.HOTEL_NO2);
				$("#hotelNo3").val(result.HOTEL_NO3);	
				$("#hotelNo4").val(result.HOTEL_NO4);
				$("#hotelNo5").val(result.HOTEL_NO5);
				$("#fNamePassport").val(result.FNAMEPASSPORT);
				$("#lNamePassport").val(result.LNAMEPASSPORT);
				$("#passportNo").val(result.PASSPORT_NO);
				$("#nationality").val(result.NATIONALITY);
				$("#pp_country").val(result.PASSPORT_ISSUE_COUNTRY);
				$("#placeOfIssue").val(result.PLACE_ISSUE);
				$("#dateOfIssue").val(result.DATE_ISSUE);
				$("#dateOfExpiry").val(result.EXPIRY_DATE);
				$("#dob").val(result.DOB);				
				$("#hidshowFlag").val(result.SHOWFLAG);
				
				$('table#tblItineraryFltMain tr.flightDataRow td.journyInner table.journyDetailFltInner2Tbl tbody tr').each(function() {
					var departClassFlt = $(this).find(".comboDepartClassFlt");
					
					if(departClassFlt.length != 0) {
						$(departClassFlt).html('');
						$(departClassFlt).get(0).options.add(new Option());
						$(departClassFlt).html(result.trvClassFltHtml);
					}
				});
				
				setTravellerPreference(true,true,true);
				...
				validateCostCentreLastName(costCenterName, lastName);
...
$("select#billingClient").live("change", function() {
	...
	var reqpage="quickTravelReqBillSMGUser";	  	
  	var Params='<%="language1="+strsesLanguage+"&suserRole="+SuserRole%>';
    var urlParams=Params+"&reqpage="+reqpage+"&billingClientId="+billingClientId+"&siteId="+siteId+"&userId="+userId;
...
function checkData(f1, actionFlag)  {
	...
	if(f1.userNameCombo.value == 'S' || f1.userNameCombo.value == '' || f1.userNameCombo.value == '-1') {
       alert('<%=dbLabelBean.getLabel("alert.global.travellername",strsesLanguage)%>');
	   f1.userNameCombo.focus();
	   return false;  
	}
	
	var loginUserId = '<%=Suser_id%>';
	var selectedUserId = $("select#userNameCombo option:selected").val();
	
	var lastName = document.getElementById("lastName").value;	
	if (lastName == "" || typeof(lastName) == 'undefined') {
		if(loginUserId == selectedUserId) {
			alert('Please update last name in your profile from "My Links -> Update Profile" page before creating request.');
		} else {
			alert('Please update last name in selected user profile from "Master -> User" page before creating request.');
		}
		f1.lastName.focus();
		return false;
	}
	...
	if (f1.costCenterName.value == "") {
		if(loginUserId == selectedUserId) {
			...
			alert('Please update cost centre in selected user profile from "Master -> User" page before save or submit to workflow.');
		}			
	...
	// Total Travel Fare section checks
		var totalfarecurrency 	= $('select#totalfarecurrency option:selected').val();
		var totalfareamount 	= $('input#totalfareamount').val();
		
		if(totalfareamount != "" && totalfarecurrency == "") {
			alert('Please select currency in Travel Fare section.');
			f1.TotalFareCurrency.focus();
			return false;
		}
		if(totalfareamount == "" && totalfarecurrency != "") {
			alert('Please enter total fare amount in Travel Fare section.');
			f1.TotalFareAmount.focus();
			return false;
		}
	// Total Travel Fare section checks
	
	//Passport section JS checks
	if($('input:radio#rd_Int').is(':checked')) {
		if(f1.fNamePassport.value == "") {
			alert('<%=dbLabelBean.getLabel("alert.global.pleaseenterfirstnames",strsesLanguage)%>');
			f1.fNamePassport.focus();
			return false;
		}
		if(f1.lNamePassport.value == "") {
			alert('<%=dbLabelBean.getLabel("alert.global.pleaseenterlastnames",strsesLanguage)%>');
			f1.lNamePassport.focus();
			return false;
		}	
		if(f1.dob.value == "") {
		...
		if(f1.placeOfIssue.value == "") {
			alert('<%=dbLabelBean.getLabel("alert.createrequest.enterplaceofissue",strsesLanguage)%>');
			f1.placeOfIssue.focus();
			return false;
		}		
		if(f1.dateOfIssue.value == "") {
			alert('<%=dbLabelBean.getLabel("alert.createrequest.enterdateofissue",strsesLanguage)%>');
			f1.dateOfIssue.focus();
			return false;
		}
		if(f1.dateOfExpiry.value == "") {
			alert('<%=dbLabelBean.getLabel("alert.createrequest.enterdateofexpiry",strsesLanguage)%>');
			f1.dateOfExpiry.focus();
			return false;
		}
		
		flag =  isDate(f1.dateOfIssue.value);
		if(flag == false) {
			f1.dateOfIssue.focus();
		  	return flag; 
		}		
		flag =  isDate(f1.dateOfExpiry.value);
		if(flag == false) {
			f1.dateOfExpiry.focus();
		  	return flag; 
		}
		
		flag = checkDateOfExpiry(f1, todayDate, f1.dateOfIssue.value, f1.dateOfExpiry.value);
	    if(flag == false) {
		...
		flag =  isDate(f1.dob.value);
		if(flag == false) {
			f1.dob.focus();
		  	return flag; 
		}
		if(!$('input:radio#visaRequired_Y').is(":checked") && !$('input:radio#visaRequired_N').is(":checked")) {
			alert('<%=dbLabelBean.getLabel("alert.createrequest.visarequired",strsesLanguage)%>');
			return false;
		}
		
	}// Passport section validation is End.
...
function validateCostCentreLastName(costCenterName, lastName) {
	var loginUserId = '<%=Suser_id%>';
	var selectedUserId = $("select#userNameCombo option:selected").val();
		
	var costCentreMsgFlag = false;	
	if (costCenterName == "" || typeof(costCenterName) == 'undefined') {
		costCentreMsgFlag = true;
	}
	...
	if(costCentreMsgFlag && lastMsgFlag) {
		if(loginUserId == selectedUserId) {
			alert('Please update cost centre and last name in your profile from "My Links -> Update Profile" page before create travel request.');
		} else {
			alert('Please update cost centre and last name in selected user profile from "Master -> User" page before creating request.');
		}
	} else if(costCentreMsgFlag) {
		if(loginUserId == selectedUserId) {
			alert('Please update cost centre in your profile from "My Links -> Update Profile" page before create travel request.');
		} else {
			alert('Please update cost centre in selected user profile from "Master -> User" page before creating request.');
		}		
	} else if(lastMsgFlag) {
		if(loginUserId == selectedUserId) {
			alert('Please update last name in your profile from "My Links -> Update Profile" page before creating request.');
